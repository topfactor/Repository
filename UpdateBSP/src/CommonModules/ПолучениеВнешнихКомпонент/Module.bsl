...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	МодульПолучениеВнешнихКомпонентВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("ПолучениеВнешнихКомпонентВМоделиСервисаПереопределяемый");
...
Функция ИнформацияОбАктуальныхВерсияхВнешнихКомпонент(ОписаниеВнешнихКомпонент)
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЧастиСтрок.Добавить(НСтр("ru = 'Не удалось получить актуальные версии внешних компонент.'"));
...
Функция ИнформацияОВерсияхВнешнихКомпонент(ОписаниеВнешнихКомпонент)
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЧастиСтрок = Новый Массив;
		ЧастиСтрок.Добавить(НСтр("ru = 'Не удалось версии внешних компонент.'"));
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(РезультатОперации.СообщениеОбОшибке);
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ОбработатьОтветСервиса(
	...
	Для Каждого ОписательВерсии Из ДанныеВнешнихКомпонент Цикл
		Если Не ЗначениеЗаполнено(ОписательВерсии.Идентификатор) Тогда
			...
			СообщениеОбОшибке = НСтр("ru = 'Не верный формат ответа Сервиса внешних компонент.'");
		...
		Если Прав(НРег(СокрЛП(СтруктураURI.Хост)), 6) <> Прав(НРег(СокрЛП(ХостСервиса)), 6) Тогда
			...
			СообщениеОбОшибке = НСтр("ru = 'Не верный адрес файла обновления внешней компоненты.'");
			ЗаписатьИнформациюВЖурналРегистрации(СообщениеОбОшибке);
			ВызватьИсключение СообщениеОбОшибке;
...
Процедура ЗагрузитьФайлыВнешнихКомпонент(РезультатОперации)
	...
	Для Каждого ОписаниеВнешнейКомпоненты Из РезультатОперации.ДанныеВнешнихКомпонент Цикл
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ПроверитьURL(ОписаниеВнешнейКомпоненты.ИдентификаторФайла);
...
Функция ПереопределитьСообщениеПользователю(КодОшибки)
	...
	Если КодОшибки = "НетДоступаКПрограмме" Тогда
		ЧастиСтрок = Новый Массив;
		ЧастиСтрок.Добавить(НСтр("ru = 'Доступ к обновлениям внешних компонент невозможен,
			|так как ваша программа не находится на официальной поддержке.
			|'"));
...
Функция ПереопределитьКодОшибкиВнешнейКомпоненты(КодОшибки)
	...
	ИначеЕсли ВРег(КодОшибки) = ВРег("Component_version_not_found") Тогда
		Возврат "ОтсутствуетВерсия";
	ИначеЕсли ВРег(КодОшибки) = ВРег("Actual_component_version_not_found") Тогда
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Получение внешних компонент.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
