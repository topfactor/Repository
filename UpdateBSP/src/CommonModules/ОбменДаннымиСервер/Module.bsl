...
Функция СохраненныйВариантНастройкиУзлаПланаОбмена(УзелПланаОбмена) Экспорт
	...
	ИмяПланаОбмена = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелПланаОбмена);
...
Процедура ПриНачалеСохраненияНастроекСинхронизации(НастройкиСинхронизации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	Обработки.ПомощникСозданияОбменаДанными.ПриНачалеСохраненияНастроекСинхронизации(НастройкиСинхронизации,
...
Процедура ПриОжиданииСохраненияНастроекСинхронизации(ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
	...
	Обработки.ПомощникСозданияОбменаДанными.ПриОжиданииСохраненияНастроекСинхронизации(ПараметрыОбработчика,
		ПродолжитьОжидание);
...
Процедура ПриЗавершенииСохраненияНастроекСинхронизации(ПараметрыОбработчика, СтатусЗавершения) Экспорт
	...
	Обработки.ПомощникСозданияОбменаДанными.ПриЗавершенииСохраненияНастроекСинхронизации(ПараметрыОбработчика,
		СтатусЗавершения);
...
// Возвращает строку недопустимых символов в имени пользователя,
// используемого для аутентификации при создании WSПрокси.
//
// Возвращаемое значение:
//	Строка - строка недопустимых символов в имени пользователя.
//
...
Функция ИдентификаторЭтогоУзлаДляОбмена(УзелОбмена) Экспорт
	...
	КодУзла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПланыОбмена[ИмяПланаОбмена].ЭтотУзел(), "Код");
...
Функция ИдентификаторУзлаКорреспондентаДляОбмена(УзелОбмена) Экспорт
	...
	Если ОбменДаннымиПовтИсп.ЭтоУзелРазделенногоОбменаДаннымиБСП(УзелОбмена) Тогда
		...
		Если СтрДлина(КодУзла) = 2
			...
			// В качестве кода для целей идентификации при обмене использует префикс,
			// установленный при создании настройки подключения.
			КодУзла = ПрефиксыУзла.ПрефиксКорреспондента;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПеренестиПаролиВБезопасноеХранилище() Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	НастройкиТранспорта.УзелИнформационнойБазы,
	|	НастройкиТранспорта.УдалитьCOMПарольПользователя,
	|	НастройкиТранспорта.УдалитьFTPСоединениеПароль,
	|	НастройкиТранспорта.УдалитьWSПароль,
	...
	Пока РезультатЗапроса.Следующий() Цикл
		Если Не ПустаяСтрока(РезультатЗапроса.УдалитьCOMПарольПользователя)
			...
			Попытка
				УстановитьПривилегированныйРежим(Истина);
				ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(РезультатЗапроса.УзелИнформационнойБазы, РезультатЗапроса.УдалитьCOMПарольПользователя, "COMПарольПользователя");
				ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(РезультатЗапроса.УзелИнформационнойБазы, РезультатЗапроса.УдалитьFTPСоединениеПароль, "FTPСоединениеПароль");
				ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(РезультатЗапроса.УзелИнформационнойБазы, РезультатЗапроса.УдалитьWSПароль, "WSПароль");
				ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(РезультатЗапроса.УзелИнформационнойБазы, РезультатЗапроса.УдалитьПарольАрхиваСообщенияОбмена, "ПарольАрхиваСообщенияОбмен");
				УстановитьПривилегированныйРежим(Ложь);
				
				СтруктураЗаписи = Новый Структура("УзелИнформационнойБазы", РезультатЗапроса.УзелИнформационнойБазы);
				СтруктураЗаписи.Вставить("УдалитьCOMПарольПользователя", "");
				СтруктураЗаписи.Вставить("УдалитьFTPСоединениеПароль", "");
				СтруктураЗаписи.Вставить("УдалитьWSПароль", "");
				СтруктураЗаписи.Вставить("УдалитьПарольАрхиваСообщенияОбмена", "");
				
				ОбновитьЗаписьВРегистрСведений(СтруктураЗаписи, "УдалитьНастройкиТранспортаОбмена");
...
Процедура УстановитьРежимВыгрузкиОбъектовДляВсехУзловИнформационнойБазы() Экспорт
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиПовтИсп.ПланыОбменаБСП() Цикл
		...
		Для Каждого Узел Из УзлыПланаОбмена(ИмяПланаОбмена) Цикл
...
Процедура ОбновитьКонстантуНастройкаПодчиненногоУзлаРИБЗавершена() Экспорт
	...
	Если  ЭтоПодчиненныйУзелРИБ()
		И РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспортаДляУзлаЗаданы(ГлавныйУзел()) Тогда
...
// Устанавливает значение реквизита WSЗапомнитьПароль в РС.УдалитьНастройкиТранспортаОбмена в значение Истина.
//
Процедура УстановитьПризнакСохраненияПароляДляОбменаЧерезИнтернет() Экспорт
	...
	|	РегистрСведений.УдалитьНастройкиТранспортаОбмена КАК НастройкиТранспорта
	|ГДЕ
	|	НастройкиТранспорта.ВидТранспортаСообщенийОбменаПоУмолчанию = ЗНАЧЕНИЕ(Перечисление.ВидыТранспортаСообщенийОбмена.WS)";
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	СостояниеОбработки = ОбновлениеИнформационнойБазы.ОбъектОбработан(ПараметрыОбмена.УзелИнформационнойБазы);
	
	Если Не СостояниеОбработки.Обработан Тогда
		РегистрыСведений.УдалитьНастройкиТранспортаОбмена.ПеренестиНастройкиТранспортаОбменаДаннымиКорреспондента(ПараметрыОбмена.УзелИнформационнойБазы);
		РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.ОбновитьОбщиеНастройкиКорреспондента(ПараметрыОбмена.УзелИнформационнойБазы);
		РегистрыСведений.НастройкиОбменаДаннымиXDTO.ОбновитьНастройкиОбменаДаннымиXDTOКорреспондента(ПараметрыОбмена.УзелИнформационнойБазы);
	КонецЕсли;
...
Процедура ВыполнитьОбменДаннымиЧерезФайловыйРесурс(СтруктураНастроекОбмена, Знач ТолькоПараметры = Ложь)
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		...
		Если СтруктураНастроекОбмена.РезультатВыполненияОбмена = Неопределено Тогда
			...
			Если СтруктураНастроекОбмена.ДополнительныеПараметры.Свойство("СообщениеДляСопоставленияДанных") Тогда
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			СтандартныеПодсистемыСервер.ПриПолученииДанныхОтГлавного(Данные,
				ПолучениеЭлемента, ОтправкаНазад, ЧтениеСообщения.Отправитель.ПолучитьОбъект());
...
Функция СобытиеЖурналаРегистрацииУдалениеОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Удаление обмена данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииРегистрацияДанныхДляНачальнойВыгрузки() Экспорт
	
	Возврат НСтр("ru = 'Обмен данными.Регистрация данных для начальной выгрузки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхДляСопоставления() Экспорт
	
	Возврат НСтр("ru = 'Обмен данными.Выгрузка данных для сопоставления'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УдалитьНеактуальныеЗаписиВРегистреПравилДляОбменаДанными()
	...
	Пока Выборка.Следующий() Цикл
		...
		НаборЗаписей = СоздатьНаборЗаписейРегистраСведений(Новый Структура("ИмяПланаОбмена", Выборка.ИмяПланаОбмена),
...
Функция ПараметрыИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла, СообщениеОбОшибке) Экспорт
	...
	Результат.Вставить("ВерсияФорматаНастроекОбменаДанными",
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ,
		СтруктураНастроек, ВидТранспорта, СообщениеОбОшибке = "", НовыйПароль = Неопределено) Экспорт
...
Функция WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы,
		СтрокаСообщенияОбОшибке = "", ПараметрыАутентификации = Неопределено, МинимальнаяВерсия = "")
	
	Если ОбменДаннымиПовтИсп.ЭтоУзелОбменаСообщениями(УзелИнформационнойБазы) Тогда
		МодульНастройкиТранспортаОбменаСообщениями = РегистрыСведений["НастройкиТранспортаОбменаСообщениями"];
		СтруктураНастроек = МодульНастройкиТранспортаОбменаСообщениями.НастройкиТранспортаWS(УзелИнформационнойБазы, ПараметрыАутентификации);
	Иначе
		СтруктураНастроек = РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспортаWS(УзелИнформационнойБазы, ПараметрыАутентификации);
	КонецЕсли;
	...
	ВерсияКорреспондента_3_0_1_1 = (ВерсииКорреспондента.Найти("3.0.1.1") <> Неопределено);
	
	Если Не ПустаяСтрока(МинимальнаяВерсия) Тогда
		Если МинимальнаяВерсия = "3.0.1.1"
			И Не ВерсияКорреспондента_3_0_1_1 Тогда
			СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ВерсияКорреспондента_3_0_1_1 Тогда
		
		WSПрокси = ПолучитьWSПрокси_3_0_1_1(СтруктураНастроек, СтрокаСообщенияОбОшибке);
		
	ИначеЕсли ВерсияКорреспондента_2_1_1_7 Тогда
...
Функция ВариантОбменаДанными(Знач Корреспондент) Экспорт
	...
	ТолькоПолучение = Истина;
	ВыгрузкаВручную = Ложь;
	
	Для Каждого Реквизит Из ЗначенияРеквизитов Цикл
		
		Если Реквизит.Значение = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			Продолжить;
		КонецЕсли;
		
		Если Реквизит.Значение <> Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
			ТолькоПолучение = Ложь;
		КонецЕсли;
		
		Если Реквизит.Значение = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьВручную Тогда
			ВыгрузкаВручную = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ТолькоПолучение Тогда
		Результат = "ТолькоПолучение";
	ИначеЕсли ВыгрузкаВручную Тогда
