...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЭтоПлатформаWindows = ЭтоПлатформаWindows();
	...
	Для Каждого ФайлБезТекста Из ФайлыДляИзвлеченияТекста Цикл
		...
		Попытка
			...
			Если ФайлОбъект = Неопределено Тогда // файл уже удален в другом сеансе
				ОтменитьТранзакцию();
				Продолжить;
			КонецЕсли;
			...
			Если Не ЭтоПлатформаWindows Тогда // Невозможно извлечь текст.
				ФайлОбъект.СтатусИзвлеченияТекста = Перечисления.СтатусыИзвлеченияТекстаФайлов.НеИзвлечен;
				ФайлОбъект.ТекстХранилище = Новый ХранилищеЗначения("");
				Продолжить;
			КонецЕсли;
		...
		Если УдалятьВременныйФайл Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Извлечение текста'",	ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьНовыеПрисоединенныеФайлы(СтрокиДереваФайлов, ТаблицаФайлов, ПараметрыСинхронизации, ОбъектВладелец = Неопределено)
	...
	Для каждого СтрокаДереваФайлов Из СтрокиДереваФайлов Цикл
		...
		Иначе 
			...
			Если ОбъектВладелец = Неопределено Или ТипЗнч(ОбъектВладелец) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
				Возврат;
			КонецЕсли;
...
Функция ВыбратьДанныеПоПравилам(УчетнаяЗапись, Синхронизировать = ИСТИНА)
	...
	Для Каждого Настройка Из НастройкиСинхронизации Цикл
		...
		Для Каждого СтрокаФайлов Из ДеревоФайлов.Строки Цикл
			...
			Если ВозможностьСоздаватьГруппы И Настройка.ЭтоНастройкаДляЭлементаСправочника Тогда
...
Функция ИзвлечьТекст(Знач АдресВременногоХранилищаТекста, Знач ДвоичныеДанные = Неопределено, Знач Расширение = Неопределено) Экспорт
	...
	Если Не ЭтоПлатформаWindows() Или ДвоичныеДанные = Неопределено Тогда
		Результат.СтатусИзвлеченияТекста = Перечисления.СтатусыИзвлеченияТекстаФайлов.НеИзвлечен;
		Результат.ТекстХранилище = Новый ХранилищеЗначения("");
		Возврат Результат;
	КонецЕсли;
...
Функция ИзвлечьТекстИзФайлаНаДиске(Знач ИмяФайла, Знач Кодировка = Неопределено) 
	
	Отказ = Ложь;
	ИзвлеченныйТекст = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекст(ИмяФайла, Отказ, Кодировка);
