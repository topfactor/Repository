		...
		Если ПараметрыВыгрузки.ВыгружаемыеТипы.Найти(ОбъектМетаданных) <> Неопределено Тогда
			...
			Для Каждого ИмяПоля Из ПравилоКонтроля.Значение Цикл
				...
				Для Каждого ПроверяемыйТипПоля Из ПроверяемыеТипыПоля Цикл
					...
					Если Не ПустаяСтрока(УсловияПоТипам) Тогда
						УсловияПоТипам = УсловияПоТипам + " ИЛИ ";
					КонецЕсли;
					
					УсловияПоТипам = УсловияПоТипам + "[ИмяПоля] ССЫЛКА " + Метаданные.НайтиПоТипу(ПроверяемыйТипПоля).ПолноеИмя();
					
				КонецЦикла;
				
				ТекстЗапроса =
					"ВЫБРАТЬ РАЗЛИЧНЫЕ ТИПЗНАЧЕНИЯ([ИмяПоля]) КАК Тип ИЗ [Таблица] ГДЕ ([УсловияПоТипам]) И [ИмяПоля].Предопределенный";
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[УсловияПоТипам]", УсловияПоТипам);
