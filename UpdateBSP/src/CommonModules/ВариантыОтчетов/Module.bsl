...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если СтрокаОтчет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра Отчет в процедуре ВариантыОтчетов.ОписаниеОтчета.
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра КлючВарианта в процедуре ВариантыОтчетов.ОписаниеВарианта:
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Для Каждого СтрокаПодсистема Из Найденные Цикл
			СсылкаПодсистемы = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(СтрокаПодсистема.ПодсистемаМетаданные);
...
// После завершения работы с настройками следует вызвать ОтчетыСервер.ОчиститьДеревоЗначений
// для очистки от циклических ссылок для корректного освобождение памяти.
//
// Возвращаемое значение:
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() И ОтчетМетаданные.РасширениеКонфигурации() <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	ОтчетыСервер.ОчиститьДеревоЗначений(Кэш.ДеревоВариантов);
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	Если Кэш.ОбщиеДанные Тогда
		ОтчетыСервер.ОчиститьДеревоЗначений(ДеревоВариантов);
	КонецЕсли;
...
Функция ОбновитьПредопределенный(Кэш, ОписаниеВарианта)
	...
	Для Каждого КлючИЗначение Из ОписаниеВарианта.Размещение Цикл
		СсылкаПодсистемы = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(КлючИЗначение.Ключ);
		Если ТипЗнч(СсылкаПодсистемы) = Тип("Строка") Тогда
			Продолжить;
		КонецЕсли;
