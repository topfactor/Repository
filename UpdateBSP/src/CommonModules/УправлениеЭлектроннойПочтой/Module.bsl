	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Пока ВыборкаПисьма.Следующий() Цикл
			...
			Попытка
				...
				Взаимодействия.ВыполнитьОтправкуПисьма(ПисьмоОбъект, Соединение, ПараметрыПисьма);
			...
			Иначе
				...
				Попытка
					...
					Если ПротоколПодключения = ПротоколИнтернетПочты.IMAP
						И Не СерверИсходящейПочтыНеТребуетДополнительнойОтправкиПоIMAP(ВыборкаПисьма.СерверИсходящейПочты) Тогда
						Взаимодействия.ВыполнитьОтправкуПисьма(ПисьмоОбъект, Соединение, ПараметрыПисьма, "IMAP");
						ИдентификаторСообщенияОтправкаIMAP = ПараметрыПисьма.ИдентификаторСообщения;
					КонецЕсли;
				...
				Исключение
					...
					Если Не ЭтоПроблемаОтвергнутыхСерверомПочтовыхАдресов Тогда
