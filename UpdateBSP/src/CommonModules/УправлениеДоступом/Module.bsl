...
// Проверяет, что чтение существующего объекта из в базы данных разрешено на уровне прав и
// на уровне записей для текущего пользователя.
//
// Важно: если подсистема работает в режиме стандартного ограничения,
// а не в режиме универсального ограничения, тогда проверяется только право на таблицу
// без проверки права на уровне записей.
// 
// Параметры:
...
// См. УправлениеДоступом.ЧтениеРазрешено.
...
Функция РазрешенныеЗначенияДляДинамическогоСписка(Таблица, ТипЗначений) Экспорт
	...
	Возврат УправлениеДоступомСлужебный.РазрешенныеЗначенияДляДинамическогоСписка(Таблица, ТипЗначений);
...
Функция ПраваПоИдентификаторам(Идентификаторы = Неопределено) Экспорт
	...
	ОбъектыМетаданныхИдентификаторов =
		Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(Идентификаторы);
...
//
// Рекомендуется при:
// - восстановление из резервной копии XML,
// - массовая загрузка данных из файла,
// - массовая загрузка данных при обмене данными,
