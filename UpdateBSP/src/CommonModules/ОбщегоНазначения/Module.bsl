...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		МестоСсылка = МестоИспользования.Ссылка;
		МестоДанные = МестоИспользования.Данные;
		МестоМетаданные = МестоИспользования.Метаданные;
		ТипДанных = ТипЗнч(МестоДанные);
		
		ИсключениеПоиска = ИсключенияПоискаСсылок[МестоМетаданные];
		
		Если ИсключениеПоиска = Неопределено Тогда
			ЭтоВспомогательныеДанные = (МестоСсылка = МестоДанные);
		...
		Иначе
			...
			Для Каждого ПутьКРеквизиту Из ИсключениеПоиска Цикл
				Если ЗначениеРеквизитаПоПути(МестоДанные, ПутьКРеквизиту) = МестоСсылка Тогда
					ЭтоВспомогательныеДанные = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Если МетаданныеДокументов.Содержит(МестоМетаданные) Тогда
			Представление = Строка(МестоДанные);
			
		ИначеЕсли МетаданныеКонстант.Содержит(МестоМетаданные) Тогда
			Представление = МестоМетаданные.Представление() + " (" + НСтр("ru = 'константа'") + ")";
			
		ИначеЕсли МетаданныеПоследовательностей.Содержит(МестоМетаданные) Тогда
			Представление = МестоМетаданные.Представление() + " (" + НСтр("ru = 'последовательность'") + ")";
		...
		ИначеЕсли ТипВсеСсылки.СодержитТип(ТипДанных) Тогда
			...
			ЗаполнитьЗначенияСвойств(МетаПредставлениеОбъекта, МестоМетаданные);
			Если ПустаяСтрока(МетаПредставлениеОбъекта.ПредставлениеОбъекта) Тогда
				МетаПредставление = МестоМетаданные.Представление();
			Иначе
		...
		ИначеЕсли ТипКлючиЗаписей.СодержитТип(ТипДанных) Тогда
			Представление = МестоМетаданные.ПредставлениеЗаписи;
			Если ПустаяСтрока(Представление) Тогда
				Представление = МестоМетаданные.Представление();
			КонецЕсли;
			
			ОписаниеИзмерений = "";
			Для Каждого КлючЗначение Из ОписаниеИзмеренийНабора(МестоМетаданные, КэшИзмеренийРегистров) Цикл
				Значение = МестоДанные[КлючЗначение.Ключ];
				Описание = КлючЗначение.Значение;
				Если Значение = МестоСсылка Тогда
...
Функция КоллекцииИдентичны(КоллекцияСтрок1, КоллекцияСтрок2, Знач ИменаКолонок = "", Знач ИсключаяКолонки = "", УчитыватьПоследовательностьСтрок = Ложь) Экспорт
	...
	Если Не СравниваютсяМассивы Тогда
		...
		Если ПустаяСтрока(ИменаКолонок) Тогда
			...
			КонецЕсли;
		Иначе
			ИменаКолонок = СтрЗаменить(ИменаКолонок, " ", "");
			СравниваемыеКолонки = СтрРазделить(ИменаКолонок, ",");
		КонецЕсли;
		
		// Вычитаем исключаемые поля
		ИсключаяКолонки = СтрЗаменить(ИсключаяКолонки, " ", "");
...
Процедура УдалитьВременныйКаталог(Знач ПутьККаталогу) Экспорт
	...
	Если Не СтрНачинаетсяС(ПутьККаталогу, КаталогВременныхФайлов()) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
//		МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодель("ОбновлениеКонфигурации");
//		МодульОбновлениеКонфигурации.<Имя метода>();
//	КонецЕсли;
//
//	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолнотекстовыйПоиск") Тогда
//		МодульПолнотекстовыйПоискСервер = ОбщегоНазначения.ОбщийМодель("ПолнотекстовыйПоискСервер");
...
Функция ЗначениеРеквизитаПоПути(Знач Данные, Знач ПутьДанных)
	Результат = Данные;
