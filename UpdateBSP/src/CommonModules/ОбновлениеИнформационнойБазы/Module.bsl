...
Функция ВыбратьИзмеренияНезависимогоРегистраСведенийДляОбработки(Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ПоляУпорядочивания.Количество() > 0 Тогда
		ТекстУпорядочивания = "";
		Для Каждого ИмяПоля Из ДополнительныеПараметры.ПоляУпорядочивания Цикл
			ТекстУпорядочивания = ТекстУпорядочивания + "ТаблицаИзменений." + ИмяПоля + ", ";
		КонецЦикла;
		ТекстЗапроса = ТекстЗапроса + "
		|УПОРЯДОЧИТЬ ПО
		| " + ТекстУпорядочивания + "ИСТИНА";
	КонецЕсли;
...
// Параметры:
//  Очередь                 - Число, Неопределено - очередь обработки, в которой выполняется
//                             текущий обработчик. Если передано Неопределенно,
//                             то проверяется во всех очередях.
//  ПолноеИмяРегистра       - Строка - имя регистра, движения по которому нужно переформировать.
//                             Например, РегистрНакопления.ТоварыНаСкладах
//  Измерения               - Строка, Массив - имя измерений, по которым нужно проверить блокировку
...
Процедура ЗарегистрироватьНовуюПодсистему(ИмяПодсистемы, НомерВерсии = "") Экспорт
	...
	Если ИндексЭлемента <> Неопределено Тогда
		Сведения.НовыеПодсистемы.Удалить(ИмяПодсистемы);
