...
Функция ИнициализироватьКомпонентыОбмена(НаправлениеОбмена) Экспорт
	...
	// Содержит набот совместно поддерживаемых объектов этой базы и базы корреспондента
...
Функция ЗаписатьВПротоколВыполнения(КомпонентыОбмена,
	...
	КомпонентыОбмена.СтрокаСообщенияОбОшибке = Стр;
	...
	ПозицияСсылки = СтрНайти(КомпонентыОбмена.СтрокаСообщенияОбОшибке, "e1cib/data/");
	Если ПозицияСсылки > 0 Тогда
		ПозицияУИД = СтрНайти(КомпонентыОбмена.СтрокаСообщенияОбОшибке, "?ref=");
	...
	Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
		Попытка
			...
			Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена
				И КомпонентыОбмена.ПоддерживаемыеОбъектыXDTO.Найти(ПравилоКонвертации.ОбъектФормата) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		ПсевдонимПредопределенногоУзла = ОбменДаннымиСервер.ПсевдонимПредопределенногоУзла(ЗаписьСообщения.Получатель);
		
		Confirmation = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(XMLБазоваяСхема(), "Confirmation"));
		Confirmation.ExchangePlan = ИмяПланаОбмена;
		
		Confirmation.To   = ОбменДаннымиСервер.ИдентификаторУзлаКорреспондентаДляОбмена(ЗаписьСообщения.Получатель);
		Если ЗначениеЗаполнено(ПсевдонимПредопределенногоУзла) Тогда
			// Требуется корректировка кода узла отправителя.
			Confirmation.From = СокрЛП(ПсевдонимПредопределенногоУзла);
		Иначе
			Confirmation.From = ОбменДаннымиСервер.ИдентификаторЭтогоУзлаДляОбмена(ЗаписьСообщения.Получатель);
		КонецЕсли;
		
		Если КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
			Confirmation.MessageNo  = 0;
			Confirmation.ReceivedNo = 0;
		Иначе
		...
		Для Каждого ВерсияФормата Из КомпонентыОбмена.НастройкиXDTO.ПоддерживаемыеВерсии Цикл
		...
		Если ЗначениеЗаполнено(ПсевдонимПредопределенногоУзла) Тогда
			...
			Header.NewFrom = ОбменДаннымиСервер.ИдентификаторЭтогоУзлаДляОбмена(ЗаписьСообщения.Получатель);
		КонецЕсли;
		
		ВыгрузитьПоддерживаемыеОбъектыФормата(Header, КомпонентыОбмена);
		
		Header.Prefix = ОбменДаннымиСервер.ПрефиксИнформационнойБазы();
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			КомпонентыОбмена.Вставить("КорреспондентПоддерживаетИдентификаторОбменаДанными", 
				КомпонентыОбмена.НастройкиXDTOКорреспондента.ПоддерживаемыеВерсии.Найти(НомерВерсииСПоддержкойИдентификатораОбменаДанными()) <> Неопределено);
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Если Не ЭтоРазделенныйОбъектМетаданных Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.'");
...
Функция ПоддерживаетсяВерсияСИдентификаторомОбменаДанными(Знач УзелИнформационнойБазы) Экспорт
	...
	ПроверяемаяВерсия = НомерВерсииСПоддержкойИдентификатораОбменаДанными();
	...
	Если УзелИнформационнойБазы = ПланыОбмена[ИмяПланаОбмена].ЭтотУзел()
		...
		ПоддерживаемыеВерсии = ВерсииФорматаОбмена(УзелИнформационнойБазы);
		
		Результат = (ПоддерживаемыеВерсии[ПроверяемаяВерсия] <> Неопределено);
	Иначе
		ВерсияФорматаКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелИнформационнойБазы, "ВерсияФорматаОбмена");
		
		Результат = (СравнитьВерсии(ВерсияФорматаКорреспондента, ПроверяемаяВерсия) >= 0);
	КонецЕсли;
...
Процедура ЗаполнитьПоддерживаемыеОбъектыXDTO(КомпонентыОбмена) Экспорт
	...
	ОбъектыБазы = Неопределено;
	ОбъектыБазыКорреспондента = Неопределено;
...
Процедура ВыгрузитьПоддерживаемыеОбъектыФормата(Header, КомпонентыОбмена)
	
	СтрокаВсеВерсии = "";
	Для Каждого AvailableVersion Из Header.AvailableVersion Цикл
		Если ПустаяСтрока(СтрокаВсеВерсии) Тогда
			СтрокаВсеВерсии = AvailableVersion;
		Иначе
			СтрокаВсеВерсии = AvailableVersion + "," + СтрокаВсеВерсии;
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаПоддерживаемыеОбъекты = КомпонентыОбмена.НастройкиXDTO.ПоддерживаемыеОбъекты.Скопировать();
	...
	РегистрыСведений.НастройкиОбменаДаннымиXDTO.ОбновитьНастройки(
		КомпонентыОбмена.УзелКорреспондента, "ПоддерживаемыеОбъекты", ТаблицаПоддерживаемыеОбъекты);
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	Если ПравилоОбработки = Неопределено Тогда
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД для объекта метаданных.
				|Объект метаданных: %3.'"),
