...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	РаботаСФайламиСлужебныйКлиентСервер.ПроверитьВозможностьЗагрузкиФайла(ФайлКлиента);
	...
	Попытка
		...
		Иначе
			...
			ПараметрыФайла.Вставить("ИмяБезРасширения",            ФайлКлиента.ИмяБезРасширения);
			ПараметрыФайла.Вставить("РасширениеБезТочки",          ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(ФайлКлиента.Расширение));
	...
	Исключение
		Результат.ТекстОшибки = РаботаСФайламиСлужебныйКлиентСервер.ОшибкаСозданияНовогоФайла(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ОткрытьПроводникСФайлом(Знач ПолноеИмяФайла) Экспорт
	...
	ОбщегоНазначенияКлиент.ОткрытьПроводник(ФайлНаДиске.ПолноеИмя);
...
#Если НЕ ВебКлиент Тогда
	...
	Если ИмяФайлаСПутем <> "" Тогда
		...
		Отказ = Ложь;
		Текст = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекст(ИмяФайлаСПутем, Отказ, Кодировка);
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса
	 Или Не ПроверятьЭлектронныеПодписиНаСервере Тогда
...
Процедура ПоместитьВыбранныеФайлыВХранилищеВебЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	РаботаСФайламиСлужебныйКлиентСервер.ПроверитьРасширениеФайлаДляЗагрузки(Расширение);
	ПараметрыФайла = Новый Структура;
	ПараметрыФайла.Вставить("ВладелецФайлов",              ВладелецФайла);
	ПараметрыФайла.Вставить("Автор",                       Неопределено);
	ПараметрыФайла.Вставить("ИмяБезРасширения",            ИмяБезРасширения);
...
Функция ВозможностьОсвободитьФайл(ОбъектСсылка,
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		...
		Возврат Ложь;
	Иначе
		Если РаботаСФайламиСлужебныйКлиентСервер.ПерсональныеНастройкиРаботыСФайлами().ЭтоПолноправныйПользователь Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция ИзвлечьТекстВоВременноеХранилище(ПолноеИмяФайла, УникальныйИдентификатор = "", Отказ = Ложь,
	...
	#Если Не ВебКлиент Тогда
		...
		Текст = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекст(ПолноеИмяФайла, Отказ, Кодировка);
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	ОбщиеНастройкиРаботыСФайлами = РаботаСФайламиСлужебныйКлиентСервер.ОбщиеНастройкиРаботыСФайлами();
	...
	Если ПараметрыВыполнения.ПереданныйПолныйПутьКФайлу = "" Тогда
		...
		Иначе
			...
			ПерсональныеНастройкиРаботыСФайлами = РаботаСФайламиСлужебныйКлиентСервер.ПерсональныеНастройкиРаботыСФайлами();
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
	...
	РаботаСФайламиСлужебныйКлиентСервер.ПроверитьРасширениеФайлаДляЗагрузки(
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	МаксРазмер =
		РаботаСФайламиСлужебныйКлиентСервер.ПерсональныеНастройкиРаботыСФайлами().МаксимальныйРазмерЛокальногоКэшаФайлов;
...
Процедура ИмпортФайловЦикл(ПараметрыВыполнения)
	...
	Для Индекс = ПараметрыВыполнения.ВыбранныеФайлыИндекс По ПараметрыВыполнения.ВыбранныеФайлыВГраница Цикл
		...
		Если ВыбранКаталог Тогда
			...
			ПараметрыВыполнения.МассивФайловЭтогоКаталога = РаботаСФайламиСлужебныйКлиентСервер.НайтиФайлыПсевдо(ПараметрыВыполнения.ПсевдоФайловаяСистема, ПараметрыВыполнения.Путь);
	...
	Если Не ПараметрыВыполнения.ПапкаУжеНайдена Тогда
		...
		Иначе		
			...
			ПараметрыВыполнения.ГруппаФайлов = РаботаСФайламиСлужебныйВызовСервера.СоздатьПапкуФайлов(
				ПараметрыВыполнения.ИмяПапки, ПараметрыВыполнения.Владелец, , ПараметрыВыполнения.ГруппаФайлов);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.УдалятьФайлыПослеДобавления = Истина Тогда
		РаботаСФайламиСлужебныйКлиентСервер.УдалитьФайлыПослеДобавления(
			ПараметрыВыполнения.МассивСтруктурВсехФайлов, ПараметрыВыполнения.МассивВсехПапок);
	КонецЕсли;
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
	...
	Для Каждого ИмяФайла Из ПараметрыПроверки.ВыбранныеФайлы Цикл
		...
		Если ВыбранКаталог Тогда
			МассивФайловЭтогоКаталога = РаботаСФайламиСлужебныйКлиентСервер.НайтиФайлыПсевдо(ПараметрыПроверки.ПсевдоФайловаяСистема, Путь);
...
Процедура Расшифровать(ОбработчикРезультата, ФайлСсылка, УникальныйИдентификатор, ДанныеФайла) Экспорт
	...
	ПараметрыВыполнения.Вставить("ИзвлекатьТекстыФайловНаСервере",
		РаботаСФайламиСлужебныйКлиентСервер.ОбщиеНастройкиРаботыСФайлами().ИзвлекатьТекстыФайловНаСервере);
...
// Продолжение процедуры (см. выше).
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеОтветаНаВопросПовторить(Ответ, ПараметрыВыполнения)
...
// Подписывает текущую версию файлов с помощью подсистемы ЭлектроннаяПодпись.
Процедура ПодписатьФайлы(МассивФайлов, ИдентификаторФормы, ОбработчикЗавершения, ОбработчикПриПолученииПодписи = Неопределено) Экспорт
	...
	Для каждого Файл Из МассивФайлов Цикл
		...
		Если ЗначениеЗаполнено(ДанныеФайла.Редактирует) Тогда
			ТекстПредупреждения = РаботаСФайламиСлужебныйКлиентСервер.СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(Файл);
			ВернутьРезультатПослеПоказаПредупреждения(ОбработчикЗавершения, ТекстПредупреждения, ПараметрыВыполнения);
			Возврат;
		КонецЕсли;
		
		Если ДанныеФайла.Зашифрован Тогда
			ТекстПредупреждения = РаботаСФайламиСлужебныйКлиентСервер.СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(Файл);
			ВернутьРезультатПослеПоказаПредупреждения(ОбработчикЗавершения, ТекстПредупреждения, ПараметрыВыполнения);
			Возврат;
		КонецЕсли;
		...
		Попытка
			...
			Если ВыбранныйФайл.ЭтоКаталог() Тогда
				...
				Если ПараметрыВыполнения.Рекурсивно = Истина Тогда
					...
					НовыйПуть = НовыйПуть + Строка(ВыбранныйФайл.Имя);
					МассивФайлов = РаботаСФайламиСлужебныйКлиентСервер.НайтиФайлыПсевдо(ПараметрыВыполнения.ПсевдоФайловаяСистема, НовыйПуть);
			...
			Если Не РаботаСФайламиСлужебныйКлиентСервер.ПроверитьВозможностьЗагрузкиФайла(
			...
			Если НЕ РаботаСФайламиСлужебныйКлиентСервер.ОбщиеНастройкиРаботыСФайлами().ИзвлекатьТекстыФайловНаСервере Тогда
...
Процедура НайтиСлишкомБольшиеФайлы(
	...
	МаксРазмерФайла = РаботаСФайламиСлужебныйКлиентСервер.ОбщиеНастройкиРаботыСФайлами().МаксимальныйРазмерФайла;
		...
		Если ВыбранныйФайл.Существует() Тогда
			...
			Если ВыбранныйФайл.ЭтоКаталог() Тогда
				...
				Если Рекурсивно Тогда
					...
					НовыйПуть = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(НовыйПуть);
					НовыйПуть = НовыйПуть + Строка(ВыбранныйФайл.Имя);
					МассивФайловВКаталоге = РаботаСФайламиСлужебныйКлиентСервер.НайтиФайлыПсевдо(ПсевдоФайловаяСистема, НовыйПуть);
...
Процедура ОткрытьФайлДиалогПоказан(ПолучитьФайл, ДополнительныеПараметры) Экспорт
	...
	Если ФайлМожноОткрывать Тогда
		...
		КонецЕсли;
		ОткрытьФайлПриложением(ДанныеФайла, ПолноеИмяФайлаНаКлиенте);
	КонецЕсли;
