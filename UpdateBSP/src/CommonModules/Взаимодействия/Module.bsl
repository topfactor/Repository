...
Процедура СоздатьИОтправитьСообщениеSMS(Сообщение) Экспорт
	...
	СообщениеSMS.Тема                    = ВзаимодействияКлиентСервер.ТемаПоТекстуСообщения(Сообщение.Текст);
...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено, ПараметрыПисьма = Неопределено, ПротоколПочты = "") Экспорт
	...
	ИначеЕсли ПротоколПочты = "Все" Тогда
		ИдентификаторПисьма = РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение(Объект.УчетнаяЗапись, ПараметрыПисьма);
		ПараметрыПисьма.Вставить("ПротоколПочты","IMAP");
		Объект.ИдентификаторСообщенияОтправкаIMAP = РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение(Объект.УчетнаяЗапись, ПараметрыПисьма);
	КонецЕсли;
	...
	Пока ВыборкаВложения.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			Для каждого ЭП Из ЭлектронныеПодписиВладельца Цикл
				СоотвВложения.Вставить(ВыборкаВложения.ПолноеНаименование 
				                       + "-DS("+ НомерСтроки + ")." 
