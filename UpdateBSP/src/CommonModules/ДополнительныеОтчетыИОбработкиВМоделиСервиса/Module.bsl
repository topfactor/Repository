...
Функция ПараметрыПодключенияИспользуемойОбработки(Знач ИспользуемаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Свойства = "ИмяОбъекта, БезопасныйРежим, ХранилищеОбработки";
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ЭтоОтчет(Ссылка) Тогда
			...
			Результат = ВнешниеОтчеты.Подключить(АдресВоВременномХранилище, ПараметрыПодключения.ИмяОбъекта, БезопасныйРежим);
			
		Иначе
			
			Результат = ВнешниеОбработки.Подключить(АдресВоВременномХранилище, ПараметрыПодключения.ИмяОбъекта, БезопасныйРежим);
