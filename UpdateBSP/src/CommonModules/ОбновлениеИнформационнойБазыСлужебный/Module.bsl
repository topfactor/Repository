	...
	Если ЕстьПраваНаОбновлениеИнформационнойБазы(УчитыватьПривилегированныйРежим, Истина) Тогда
		Если ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
				...
				Возврат НСтр("ru = 'Вход в программу временно невозможен в связи с обновлением на новую версию.
								|Для завершения обновления версии программы требуются запуск в режиме толстого, тонкого или ВЕБ-клиента'");
				
			ИначеЕсли Не Результат.МожноВыполнятьОбновление Тогда
...
//
Функция ДобавитьПотокРегистрацииДанныхОтложенногоОбновления(ОписаниеПотоков, ОписаниеПроцедуры)
	...
	Поток.ПараметрыПроцедуры = ОписаниеПроцедуры;
	Поток.ПараметрыПроцедурыЗавершения = ОписаниеПроцедуры;
...
//  СведенияОбОбновлении - сведения об обновлении (см. НовыеСведенияОбОбновлении()).
//
Процедура ЗавершитьРегистрациюДанныхОтложенногоОбновления(ОписаниеПроцедуры,
                                                          АдресРезультата,
	...
	СведенияОбОбновлении.ОписаниеПроцедурЗаполнения[ОписаниеПроцедуры.ИмяОбработчика].Статус = "Выполнено";
	СведенияОбОбновлении.ОбрабатываемыеДанные.Вставить(ОписаниеПроцедуры.ИмяОбработчика, Результат.ДанныеОбработчика);
...
Функция ДобавитьПотокОбработчикаОтложенногоОбновления(ОписаниеПотоков, СведенияОбОбновлении)
	...
	Если ТипЗнч(ОбработчикОбновления) = Тип("СтрокаДереваЗначений") Тогда
		Если КонтекстОбработчика.ВыполнитьОбработчик Тогда
			...
			Поток.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонНаименования,
				ОбработчикОбновления.ИмяОбработчика);
		Иначе
			Поток = Истина;
		КонецЕсли;
	Иначе
...
Процедура ЗавершитьВыполнениеОтложенногоОбработчика(КонтекстОбработчика, АдресРезультата, СведенияОбОбновлении) Экспорт
	...
	ПараметрыСеанса.ПараметрыОбработчикаОбновления = КонтекстОбработчика.ПараметрыОбработчикаОбновления;
	ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ДеревоОбработчиков);
	ПараметрыСеанса.ПараметрыОбработчикаОбновления = Новый ФиксированнаяСтруктура(НовыеПараметрыОбработчикаОбновления());
...
Процедура ВыполнитьПоток(ОписаниеПотоков, Поток, ИдентификаторФормы = Неопределено)
	...
	Если Не ПустаяСтрока(ОписаниеПотоков.Процедура) И Поток.ПараметрыПроцедуры <> Неопределено Тогда
		...
		РезультатЗапуска = ДлительныеОперации.ВыполнитьВФоне(ОписаниеПотоков.Процедура,
	...
	Пока Индекс >= 0 Цикл
		...
		Если ИдентификаторЗадания <> Неопределено Тогда
			...
			Исключение
				...
				Если Не ПустаяСтрока(ОписаниеПотоков.ПриАварийномЗавершении) Тогда
					ПараметрыВызова = Новый Массив;
					ПараметрыВызова.Добавить(Поток);
					ПараметрыВызова.Добавить(ИнформацияОбОшибке);
					ПараметрыВызова.Добавить(Параметры);
					
					ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотоков.ПриАварийномЗавершении, ПараметрыВызова);
				Иначе
					ВызватьИсключение;
				КонецЕсли;
			КонецПопытки;
		...
		Если ИдентификаторЗадания = Неопределено Или ЗаданиеВыполнено <> Ложь Тогда
			Выполнять = Не ПустаяСтрока(ОписаниеПотоков.ПроцедураЗавершения)
			...
			Если Выполнять Тогда
				...
				ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотоков.ПроцедураЗавершения, ПараметрыВызова);
			КонецЕсли;
	...
	Если ОписаниеПотоков <> Неопределено Тогда
		...
		Если Потоки <> Неопределено Тогда
			...
			Пока Индекс >= 0 Цикл
				...
				Если ОписаниеПотоков.ПриОтменеПотока <> Неопределено Тогда
					ПараметрыВызова = Новый Массив;
					ПараметрыВызова.Добавить(Поток);
					ПараметрыВызова.Добавить(ПараметрыОтмены);
					ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПотоков.ПриОтменеПотока, ПараметрыВызова);
				КонецЕсли;
...
//
// Возвращаемое значение:
//  Структура - общее описание потоков и описание каждого отдельного потока со следующими полями:
			...
			Иначе
				...
				Для Каждого ОписаниеОбработчика Из ОписаниеЦиклаОбновления.Обработчики Цикл
					...
					Если ОбработчикОбновления.ЧислоПопыток >= 3
					   И ВсеОбработчикиЗациклились(СведенияОбОбновлении.ДеревоОбработчиков) Тогда
						ОтметитьЗациклившиесяОбработчики(СведенияОбОбновлении.ДеревоОбработчиков);
						ЕстьОшибки = Истина;
						Продолжить;
					КонецЕсли;
...
//  ОбработчикОбновления - СтрокаДереваЗначений - обработчик обновления в виде строки дерева обработчиков.
//
Процедура ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ДеревоОбработчиков)
	...
	Попытка
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не КонтекстОбработчика.ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 И ВсеОбработчикиЗациклились(ДеревоОбработчиков) Тогда
...
Процедура ЗапускРегистрацииДанныхОтложенногоОбновления(ИдентификаторФормы, АдресРезультата) Экспорт
	...
	Попытка
		Для каждого ТекущаяПроцедура Из СведенияОбОбновлении.ОписаниеПроцедурЗаполнения Цикл
			Поток = ДобавитьПотокРегистрацииДанныхОтложенногоОбновления(ОписаниеПотоков, ТекущаяПроцедура.Значение);
...
//  АдресРезультата - Строка - адрес временного хранилища для сохранения результата выполнения процедуры.
//
Процедура ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеПроцедуры, АдресРезультата) Экспорт
	...
	ПараметрыОтметкиКОбработке.Очередь = ОписаниеПроцедуры.Очередь;
	...
	Попытка
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ОписаниеПроцедуры.ПроцедураЗаполнения, ПараметрыОбработчика);
	Исключение
...
//
// Возвращаемое значение:
//  Булево - все выполняемые обработчики зациклились (ЧислоПопыток >= 2), Ложь - хоть один еще выполняется.
//
Процедура ОтметитьЗациклившиесяОбработчики(ДеревоОбработчиков)
	...
	Для каждого ОбработчикОбновления Из Выполняющиеся Цикл
		Если ОбработчикОбновления.ЧислоПопыток >= 2 Тогда
			ОбработчикОбновления.Статус = "Ошибка";
...
//
Функция ВсеОбработчикиЗациклились(ДеревоОбработчиков)
