				...
				Если ВидКонтактнойИнформации = Неопределено 
					...
					Если ПустаяСтрока(НоваяСтрока.Значение)
						 И ЗначениеЗаполнено(НоваяСтрока.ЗначенияПолей) Тогда
							НоваяСтрока.Значение = КонтактнаяИнформацияВJSON(НоваяСтрока.ЗначенияПолей);
					КонецЕсли;
...
//  ЗначениеИлиПредставление - Строка - Представление, JSON или XML соответствующего XDTO пакету КонтактнаяИнформация
//                                      или Адрес.
...
Процедура ДобавитьКонтактнуюИнформацию(СсылкаИлиОбъект, ЗначениеИлиПредставление, ВидКонтактнойИнформации, Дата = Неопределено, Замещать = Истина) Экспорт
	...
	Иначе
		...
		Если ЭтоКонтактнаяИнформацияВXML Тогда
			...
			ОбъектКонтактнойИнформации = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(Значение);
			Представление = ОбъектКонтактнойИнформации.Value;
...
//
// Параметры:
//    КонтактнаяИнформация - Строка, Структура, Соответствие, СписокЗначений - Описание полей контактной информации.
...
Функция КонтактнаяИнформацияВJSON(Знач КонтактнаяИнформация, Знач ОжидаемыйВид = Неопределено) Экспорт
	...
	КонтактнаяИнформацияПоПолям = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, ОжидаемыйВид);
...
//
// Параметры:
//    КонтактнаяИнформация - Строка - Контактная информация в виде XML, соответсвующего структуре 
//                                    XDTO пакетов КонтактнаяИнформация и Адрес.
//
// Возвращаемое значение:
...
Функция ТипКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(КонтактнаяИнформация) Тогда
		...
		КонтактнаяИнформацияСтруктурой = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(КонтактнаяИнформация);
...
//
// Параметры:
//    КонтактнаяИнформация - Строка, ОбъектXDTO - Строка JSON или XML контактной информации соответствующая
//                                                     XDTO пакетам КонтактнаяИнформация или Адрес.
...
Функция ДоменАдресаКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	Если КонтактнаяИнформацияСтруктурой.Свойство("Type") И КонтактнаяИнформацияСтруктурой.Свойство("Value") Тогда
		...
		Если Строка(КонтактнаяИнформацияСтруктурой.Type) = Строка(Перечисления.ТипыКонтактнойИнформации.ВебСтраница) Тогда
		...
		ИначеЕсли Строка(КонтактнаяИнформацияСтруктурой.Type) = Строка(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты) Тогда
		...
		Если ПустаяСтрока(СтрокаКИ.Значение) Тогда
			Если ЗначениеЗаполнено(СтрокаКИ.ЗначенияПолей) Тогда
				...
				КонтактнаяИнформацияПоПолям = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(СтрокаКИ.ЗначенияПолей,
					СтрокаКИ.Тип, СтрокаКИ.Представление);
		...
		Если Результат.Свойство("ИмяЭлементаДляРазмещения") Тогда
			...
			Если Результат.Свойство("ИмяРеквизита") Тогда
				...
				Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(НайденнаяСтрока.Значение) Тогда
					КонтактнаяИнформацияПоПолям = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(НайденнаяСтрока.Значение);
...
Процедура УстановитьСвойстваВидаКонтактнойИнформации(Параметры) Экспорт
	...
	Объект.МеждународныйФорматАдреса            = Параметры.МеждународныйФорматАдреса;
...
Процедура ЗаписатьКонтактнуюИнформацию(Объект, Знач Значение, ВидИнформации, ТипИнформации, ИдентификаторСтроки = 0, Дата = Неопределено) Экспорт
	...
	Иначе
		ОбъектКИ = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(Значение);
	КонецЕсли;
...
//
// Параметры:
//  КонтактнаяИнформация - Строка - адрес во внутреннем формате JSON или в XML, соответствующем XDTO пакету Адрес.
// 
// Возвращаемое значение:
//  Структура - сведения об телефона:
...
//
Функция СведенияОТелефоне(КонтактнаяИнформация) Экспорт
	
	ТелефонаПоПолям         = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Перечисления.ТипыКонтактнойИнформации.Телефон);
	Результат               = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураПолейТелефона();
	Результат.Представление = ТелефонаПоПолям.Value;
	Результат.КодСтраны     = ТелефонаПоПолям.CountryCode;
	Результат.КодГорода     = ТелефонаПоПолям.AreaCode;
	Результат.НомерТелефона = ТелефонаПоПолям.Number;
	Результат.Добавочный    = ТелефонаПоПолям.ExtNumber;
	Результат.Комментарий   = ТелефонаПоПолям.Comment;
			...
			Если Индекс = Неопределено Тогда
				...
				Если Не ОтложеннаяИнициализация Тогда
					ДобавляемыеРеквизиты.Добавить(
						Новый РеквизитФормы(ОбъектКонтактнойИнформации.ИмяРеквизита, Строка1500, , ОбъектКонтактнойИнформации.Наименование, Истина));
				КонецЕсли;
			Иначе
...
Функция ОшибкиЗаполненияЭлектроннойПочты(АдресЭП, ВидИнформации, Знач ИмяРеквизита = "", ПолеРеквизита = "")
	...
	СтрокаОшибки = "";
	ЭлектроннаяПочта = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(АдресЭП);
...
Процедура СкорректироватьКонтактнуюИнформацию(Форма, СтрокаКИ)
	...
	Если ПустаяСтрока(СтрокаКИ.Представление) И ЗначениеЗаполнено(СтрокаКИ.Значение) Тогда
...
Процедура ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(СтрокаКонтактнойИнформацииОбъекта, Периодическая, СтрокаКонтактнойИнформации)
	...
	ОбъектКонтактнойИнформации = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(СтрокаКонтактнойИнформации.Значение);
...
Процедура ВосстановитьПустыеЗначениеПредставление(СтрокаКонтактнойИнформации)
	...
	ЗначенияПолей = Новый Структура("ЗначенияПолей", Неопределено);
	ЗаполнитьЗначенияСвойств(ЗначенияПолей, СтрокаКонтактнойИнформации);
	ЕстьЗначенияПолей = (ТипЗнч(ЗначенияПолей.ЗначенияПолей) = Тип("Строка"));
	ПустыеЗначенияПолей = ЕстьЗначенияПолей И ПустаяСтрока(ЗначенияПолей.ЗначенияПолей);
	
	Если ЕстьЗначенияПолей Тогда
		Если ПустоеПредставление И ПустоеЗначение И ПустыеЗначенияПолей Тогда
			Возврат;
		КонецЕсли;
	Иначе
		Если ПустоеПредставление И ПустоеЗначение Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПустоеПредставление Тогда
		...
		СтрокаКонтактнойИнформации.Представление = УправлениеКонтактнойИнформациейСлужебный.ПредставлениеКонтактнойИнформации(
			СтрокаКонтактнойИнформации.ЗначенияПолей, ФорматКонтактнойИнформации);
		
	КонецЕсли;
	
	Если ПустоеЗначение Тогда
		
		Если ЕстьЗначенияПолей И ПустыеЗначенияПолей Тогда
		...
		ИначеЕсли ЕстьЗначенияПолей Тогда
	...
	ИначеЕсли ЕстьЗначенияПолей И ПустыеЗначенияПолей Тогда
		
		КонтактнаяИнформацияXDTO = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOПоПредставлению(
			СтрокаКонтактнойИнформации.Представление, СтрокаКонтактнойИнформации.Вид);
		СтрокаКонтактнойИнформации.ЗначенияПолей = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(
			КонтактнаяИнформацияXDTO);
