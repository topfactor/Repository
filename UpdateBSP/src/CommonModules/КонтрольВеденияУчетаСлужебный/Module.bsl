...
Процедура ОбновитьПараметрыПроверокУчета(ЕстьИзменения = Неопределено) Экспорт
	...
	ХэшированиеСистемныхПроверок = ХэшПроверок();
...
Функция СводнаяИнформацияПоВидамПроверок(ВидПроверок, ПоискПоТочномуСоответствию = Истина) Экспорт
	...
	ИначеЕсли ТипЗнч(ВидПроверок) = Тип("Строка") Тогда
		...
		МассивВидовПроверок         = ВидыПроверок(ПараметрыВыполненияПроверки, ПоискПоТочномуСоответствию);
	Иначе
		
		КоличествоПараметров = ВидПроверок.Количество();
		Если КоличествоПараметров = 1 Тогда
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0));
		ИначеЕсли КоличествоПараметров = 2 Тогда
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0), ВидПроверок.Получить(1));
		ИначеЕсли КоличествоПараметров = 3 Тогда
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0),
				ВидПроверок.Получить(1), ВидПроверок.Получить(2));
		ИначеЕсли КоличествоПараметров = 4 Тогда
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0),
				ВидПроверок.Получить(1), ВидПроверок.Получить(2), ВидПроверок.Получить(3));
		ИначеЕсли КоличествоПараметров = 5 Тогда
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0),
				ВидПроверок.Получить(1), ВидПроверок.Получить(2), ВидПроверок.Получить(3), ВидПроверок.Получить(4));
		Иначе
			МассивОстатков = Новый Массив;
			Для Индекс = 5 По КоличествоПараметров Цикл
				МассивОстатков.Добавить(ВидПроверок.Получить(Индекс));
			КонецЦикла;
			ПараметрыВыполненияПроверки = ПараметрыВыполненияПроверки(ВидПроверок.Получить(0),
				ВидПроверок.Получить(1), ВидПроверок.Получить(2), ВидПроверок.Получить(3), ВидПроверок.Получить(4), МассивОстатков);
		КонецЕсли;
...
// Формирует хэш таблицы проверок, для сравнения с предыдущим хешем,
...
//
// Возвращаемое значение:
//     ФиксированноеСоответствие - хэш-сумма.
//
Функция ХэшПроверок()
	...
	ДанныеПроверок.Вставить(ОбщегоНазначения.КонтрольнаяСуммаСтрокой(Новый ФиксированныйМассив(МассивХэша)));
...
Процедура ПроверитьСсылочнуюЦелостность(Проверка, ПараметрыПроверки) Экспорт
	...
	ПараметрыПроверки.Вставить("КэшСсылок", Новый Соответствие);
	...
	Для Каждого ВидМетаданных Из РегистрыКакОбъектыМетаданных() Цикл
		...
		КонецЦикла;
	КонецЦикла;
	ПараметрыПроверки.Удалить("КэшСсылок");
...
Процедура НайтиБитыеСсылки(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
	...
	РазмерКэшаПроверенныхСсылок = РазмерКэшаПроверенныхСсылок();
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Для Каждого СсылочныйРеквизит Из Ссылки Цикл
				...
				Если ПараметрыПроверки.КэшСсылок[ТекущаяСсылка] <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				...
				Иначе
					Если ПараметрыПроверки.КэшСсылок.Количество() = РазмерКэшаПроверенныхСсылок Тогда
						ПараметрыПроверки.КэшСсылок.Очистить();
					КонецЕсли;
					ПараметрыПроверки.КэшСсылок.Вставить(ТекущаяСсылка, "");
				КонецЕсли;
...
Процедура НайтиБитыеСсылкиВРегистрах(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоРегистрБухгалтерии(ОбъектМетаданных) Тогда
		НайтиБитыеСсылкиВРегистрахБухгалтерии(ОбъектМетаданных, ПараметрыПроверки, ВидыСубконто(ОбъектМетаданных), КэшБитыхСсылок);
...
Процедура НайтиБитыеСсылкиВРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
	...
	Если ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
		НайтиБитыеСсылкиВПодчиненныхРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок);
	Иначе
		Если ОбъектМетаданных.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
			НайтиБитыеСсылкиВНезависимыхПериодическихРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок);
		Иначе
			НайтиБитыеСсылкиВНезависимыхНеПериодическихРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура НайтиБитыеСсылкиВПодчиненныхРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
...
Процедура НайтиБитыеСсылкиВНезависимыхПериодическихРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Для Каждого ИнформацияОРеквизите Из ИнформацияОРегистре Цикл
				...
				Если Не ОбнаруженаБитаяСсылка Тогда
					Продолжить;
				КонецЕсли;
...
Процедура НайтиБитыеСсылкиВНезависимыхНеПериодическихРегистрахСведений(ОбъектМетаданных, ПараметрыПроверки, КэшБитыхСсылок)
...
Процедура НайтиБитыеСсылкиВРегистрахБухгалтерии(ОбъектМетаданных, ПараметрыПроверки, ВидыСубконто, КэшБитыхСсылок)
	...
	Если Не КорреспонденцияОбъекта Тогда
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Для Каждого ТекущаяЗапись Из ТекущийНаборЗаписей Цикл
				...
				Для Каждого ПроверяемыйОбъект Из ПроверяемыеОбъекты Цикл
					...
					Если Не ОбщегоНазначения.ЗначениеСсылочногоТипа(ПроверяемыйОбъект) Или Не ЗначениеЗаполнено(ПроверяемыйОбъект) Тогда
						Продолжить;
					КонецЕсли;
					
					Если ПараметрыПроверки.КэшСсылок[ПроверяемыйОбъект] <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
					
					ОбнаруженаБитаяСсылка = КэшБитыхСсылок.Получить(ПроверяемыйОбъект) <> Неопределено;
					Если Не ОбнаруженаБитаяСсылка И Не ОбщегоНазначения.СсылкаСуществует(ПроверяемыйОбъект) Тогда
						КэшБитыхСсылок.Вставить(ПроверяемыйОбъект, "");
						ОбнаруженаБитаяСсылка = Истина;
					КонецЕсли;
					...
					Если Не ОбнаруженаБитаяСсылка Тогда
						Если ПараметрыПроверки.КэшСсылок.Количество() = РазмерКэшаПроверенныхСсылок Тогда
							ПараметрыПроверки.КэшСсылок.Очистить();
						КонецЕсли;
						ПараметрыПроверки.КэшСсылок.Вставить(ПроверяемыйОбъект, "");
						Продолжить;
					КонецЕсли;
...
Функция РазмерКэшаПроверенныхСсылок()
