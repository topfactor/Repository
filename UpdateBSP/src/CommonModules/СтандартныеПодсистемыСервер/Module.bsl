...
Процедура ПроверитьВозможностьЗапуска()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрофилиБезопасности") Тогда
		ПрофильИнформационнойБазы = СтандартныеПодсистемыБезопасныйРежим.ПрофильБезопасностиИнформационнойБазы();
	Иначе
...
Функция ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти()
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент()
		ИЛИ ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент()
