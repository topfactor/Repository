...
Функция ИнформацияОбИсправлениях(Параметры, КаталогВременныхФайлов)
	...
	Если Параметры.Свойство("ФайлыИсправлений") Тогда
		УстанавливаемыеИсправления = ИменаФайловИсправлений(Параметры);
	ИначеЕсли Параметры.Свойство("Исправления") Тогда
		Если Параметры.Исправления.Свойство("Установить")
			...
			Для Каждого НовоеИсправление Из Параметры.Исправления.Установить Цикл
				ИмяФайла = СтрЗаменить(Строка(Новый УникальныйИдентификатор),"-", "") + ".cfe";
				ПолноеИмяФайла = КаталогВременныхФайлов + ИмяФайла;
				ПолучитьФайл(НовоеИсправление, ПолноеИмяФайла, Ложь);
				ИменаФайлов.Добавить(Форматировать(ПолноеИмяФайла));
			КонецЦикла;
...
Функция ИменаФайловИсправлений(Параметры)
