...
Процедура УстановитьИдентификаторыАдреса(Адрес) Экспорт
	...
	Если ТипЗнч(КодРегиона) = Тип("Число") Тогда
		Если ЗагруженныеАдресныеСведения.Получить(КодРегиона) <> Неопределено
...
Функция РаспознатьАдрес(ЧастиАдреса) Экспорт
	...
	Для каждого ЭлементАдреса Из ДомаЗдания Цикл
			...
			ИначеЕсли КодыДомовВладенийИСтроений.Строения[Тип] <> Неопределено Тогда
				Строения.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(ТРег(ЭлементАдреса.Наименование), Значение));
				...
				КонецЕсли;
			Иначе
				Квартиры.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(ТРег(ЭлементАдреса.Наименование), Значение));
			КонецЕсли;
	...
	Индекс       = ?(СтрокаИндекс <> Неопределено, СтрокаИндекс.Значение, Неопределено);
	
	Сведения = Новый Структура("ЗагруженныеДанные", Истина);
	ВариантыАдресов = НайденныеАдреса.Выгрузить();
	Если ВариантыАдресов.Количество() = 1 Или Индекс = Неопределено Тогда
	...
	Адрес.AddressType = ?(Муниципальный,
		АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес(),
		АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес());
	...
	Иначе
		...
		Для каждого ЭлементАдреса Из ЧастиАдреса Цикл
			...
			ИначеЕсли КодыДомовВладенийИСтроений.Строения[ВРег(Тип)] <> Неопределено Тогда
				Адрес.Buildings.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(ТРег(Тип), СокрЛП(СтрЗаменить(ЭлементАдреса.Сокращение, "№", ""))));
			ИначеЕсли Помещения[ВРег(Тип)] <> Неопределено Тогда
				Адрес.Apartments.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(ТРег(Тип), СокрЛП(СтрЗаменить(ЭлементАдреса.Сокращение, "№", ""))));
			КонецЕсли;
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			Для каждого СтрокаРезультатаВебСервис Из РезультатВебСервис.Данные Цикл
				...
				НоваяСтрока.Идентификатор  = СтрокаРезультатаВебСервис.id;
				НоваяСтрока.Представление  = СтрокаРезультатаВебСервис.value;
...
Функция АдресныеОбъектыУровня(Родитель, Уровень, ТипАдреса, Знач СтрокаПоиска = "") Экспорт
	...
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] Тогда
		...
		Для каждого СведенияОбАдресе Из Результат.Данные Цикл
			ДанныеВыбора.Данные.Добавить(СведенияОбАдресе.Идентификатор, СведенияОбАдресе.Представление);
		КонецЦикла;
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
				ДанныеВыбора.Данные.Добавить(СтрокаДанных.id, СтрокаДанных.value);
			КонецЦикла;
...
Функция ВариантыАвтоподбора(Текст, ДополнительныеПараметры) Экспорт
	...
	ЗагруженныеАдресныеСведения = АдресныйКлассификаторПовтИсп.СведенияОЗагрузкеСубъектовРФ();
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] Тогда
		ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, КешПредставлений, Текст, ДополнительныеПараметры);
	...
	Если ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(Результат, КешПредставлений, Текст, Родитель, 20, ДополнительныеПараметры);
	КонецЕсли;
...
Функция СписокДомов(ИдентификаторАдресногоОбъекта, Знач СтрокаПоиска, ПорцияПриПоиске = 20) Экспорт
	...
	Если ВариантыДомов.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Для каждого СведенияОДоме Из РезультатВебСервис.Данные Цикл
			ДомЧислом = Формат(НомерДомаЧислом(СведенияОДоме.number), "ЧЦ=4; ЧН=; ЧВН=; ЧГ=0; ЧФ=1Ч");
...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	ИначеЕсли ЭтоКонтактнаяИнформацияВJSON(Адрес) Тогда
		АдресСтруктура = СтрокаJSONВСтруктуру(Адрес);
	КонецЕсли;
	
	ИдентификаторАдресногоОбъекта = ?(ЗначениеЗаполнено(АдресСтруктура.ID), АдресСтруктура.ID, "");
...
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Знач ИдентификаторАдресногоОбъекта = "", Источник = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ИдентификаторАдресногоОбъекта) Тогда
		Если Не ЗначениеЗаполнено(Адрес.ID) Тогда
			УстановитьИдентификаторыАдреса(Адрес);
		КонецЕсли;
	...
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] И Источник <> "Сервис1С" Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторАдресногоОбъекта) Тогда
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Результат.КодыАдреса.КодКЛАДР = РезультатЗапроса.Выгрузить()[0].КодКЛАДР;
	...
	Если ЗагруженныеАдресныеСведения["КлассификаторДоступен"] Тогда
		...
		Если НЕ РезультатВебСервис.Отказ И РезультатВебСервис.Данные.Количество() > 0 Тогда
			...
			Если Не ОсновныеКодыЗаполнены Тогда
...
//
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина, ЗагружатьИсториюИзменений = Ложь) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Попытка
			...
			ЗагрузитьДополнительныеСведения(СубъектРФ, КаталогФайлов, СлужебныеАдресныеСведения.Сведения, ЗагружатьИсториюИзменений);
...
Процедура УстановитьИдентификаторыАдресаПоЗагруженнымДанным(Адрес)
	...
	Если АдресныйКлассификаторКлиентСервер.ЭтоМуниципальныйАдрес(Адрес.AddressType) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ".РодительскийИдентификатор", ".МуниципальныйРодительскийИдентификатор");
	КонецЕсли;
	
	ИменаУровнейАдреса = АдресныйКлассификаторКлиентСервер.ИменаУровнейАдреса(Адрес.AddressType, Истина);
	АдресныеСведенияДляЗапроса = Новый Массив;
...
Процедура УстановитьИдентификаторыАдресаЧерезВебСервис(Адрес)
	...
	Если Результат.Отказ = Ложь Тогда
		...
		Если Результат.Данные.Количество() > 0 И Результат.Данные[0].АдресПроверен  Тогда
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Значение)
	...
	Результат = ОписаниеАдреса();
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ПредставлениеМуниципальный", АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес());
	Запрос.УстановитьПараметр("ПредставлениеАдминистративный", АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес());
	...
	Если Значение.Свойство("Муниципальный") Тогда
		Результат["addressType"] = ?(Значение.Муниципальный, АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес(), АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес());
	Иначе
		Результат["addressType"] = АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес();
	КонецЕсли;
	
	Если Результат["addressType"] = АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес() И Выборка.Количество() = 2 Тогда
...
//
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, КешПредставлений, Знач Текст, ДополнительныеПараметры)
	
	ПреобразоватьВводАдреса(Текст);
	ТекстПоиска = ЗамаскироватьСпецсимволыПодобия(Текст);
	СписокФраз = СтрРазделить(ТекстПоиска, " ");
	...
	Для Каждого Строка Из РезультатЗапроса.Выгрузить() Цикл
		...
		ДобавитьВариантАдреса(Результат, КешПредставлений, Представление, Строка.Идентификатор, Строка.Муниципальный, Истина);
...
Функция ОдноуровневыйАвтоподборНаселенногоПункта(ТипаАдреса, Уровень, Порция = 20)
	...
	Если АдресныйКлассификаторКлиентСервер.ЭтоМуниципальныйАдрес(ТипаАдреса) Тогда
...
// 
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораВнутр(Результат, Индекс, ДополнительныеПараметры)
	...
	Результат.Данные = Запрос.Выполнить().Выгрузить();
...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Родитель, Уровень, ТипАдреса, ДополнительныеПараметры)
	...
	Если АдресныйКлассификаторКлиентСервер.ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РодительскийИдентификатор", "МуниципальныйРодительскийИдентификатор");
	КонецЕсли;
...
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, Знач АдресаДляПроверки)
	...
	ПрефиксВерсииЗапроса = АдресныйКлассификаторКлиентСервер.ПрефиксВерсииЗапроса();
	
	HTTPЗапрос = Новый HTTPЗапрос(ПрефиксВерсииЗапроса + "verify");
	
	ДанныеЗапроса = Новый Массив();
	ДанныеЗапроса.Добавить(АдресДляПередачиВСервис(АдресаДляПроверки.Адрес));
	...
	Если РезультатПроверки.КодСостояния = 200 Тогда
		Данные = СтрокаJSONВСтруктуру(ПолученныеДанные);
		
	Иначе
		ТекстОшибки = НСтр("ru='При получении данных из веб-сервиса возникли проблемы. Сервер вернул код %1. %2'");
		Результат.ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, РезультатПроверки.КодСостояния, ПолученныеДанные);
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='При получении данных из веб-сервиса возникли проблемы.'");
		Результат.Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементПроверки Из Данные Цикл
		...
		РезультатПроверки.id = ЭлементПроверки.id;
		
		Если ЭлементПроверки.Свойство("status") Тогда
			РезультатПроверки.АдресПроверен = Булево(ЭлементПроверки.status);
		КонецЕсли;
		...
		Если ЭлементПроверки.Свойство("errorMessage") И ЗначениеЗаполнено(ЭлементПроверки.errorMessage) Тогда
			...
			ОписаниеОшибки.Ключ = ЭлементПроверки.errorField;
...
Функция АдресДляСохраненияВБазе(Адрес, Муниципальный = Истина)
	...
	Если Муниципальный Тогда
		Результат.addressType = АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес();
	Иначе 
		Результат.addressType = АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес();
	КонецЕсли;
...
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораСервис1С(Результат, Индекс)
	...
	Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
		...
		Если ПредставлениеТерриториальное <> ПредставлениеМуниципальное Тогда
			НоваяСтрока = Результат.Данные.Добавить();
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(ВариантыАдреса, КешПредставлений, Текст, Родитель, Количество, 
	...
	Для каждого Данные Из РезультатВебСервис.Данные Цикл
		...
		ДобавитьВариантАдреса(
			ВариантыАдреса, 
			КешПредставлений, 
			ПредставлениеМуниципальное, 
			Данные.id, 
			Истина, // Муниципальный
			Ложь, // ЗагруженныеДанные
...
Процедура ДобавитьВариантАдреса(ВариантыАдреса, КешПредставлений, Представление, Идентификатор, Муниципальный, ЗагруженныеДанные, 
	Данные = "")
	
	// Такой адрес уже есть в списке выбора.
	Если КешПредставлений[Представление] <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	КешПредставлений[Представление] = Истина;
	...
	Если Не Муниципальный Тогда
		Представление = Новый ФорматированнаяСтрока(Представление,, ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	КонецЕсли;
		
	ВариантыАдреса.Добавить(Сведения, Представление);
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес)
	...
	РегионЗагруженВАдресныеСведения = СведенияОРегионе(Адрес.Area + " " + Адрес.AreaType).Загружен;
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Регион ""%1"" не существует'"), Адрес.Area + " " + Адрес.AreaType));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru = 'Отсутствуют адресные сведения по региону'") + ": " + Адрес.Area + " " + Адрес.AreaType + ".");
...
|		ПО (ДополнительныеСведения.Идентификатор = АдресныйОбъект.ДополнительныеАдресныеСведения)";
	...
	Адреса                 = РезультатЗапроса[0];
	Дома                   = РезультатЗапроса[1];
	АдресныйОбъект         = РезультатЗапроса[2];
	...
	Для каждого АдресСтрока Из Адреса.Выгрузить() Цикл
		...
		Если КакиеУровниПроверять["Distict"] <> Неопределено И АдресСтрока["Наименование" + КакиеУровниПроверять["Distict"]] = NULL Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'"), Адрес.District + " " + Адрес.DistrictType));
		КонецЕсли;
	...
	ИдентификаторДополнительныхСведений = Неопределено;
	
	ПохожиеВарианты = Новый Массив;
						...
						Если СтрСравнить(ТипДомаАдрес, НазваниеВладения) = 0 Тогда
							...
							Если СтрСравнить(НомерКорпусаАдрес, Корпус) = 0
								...
								И СтрСравнить(ТипСтроенияАдрес, НазваниеСтроения) = 0 Тогда
									...
									ИдентификаторДополнительныхСведений = ИнформацияОДомах.ДополнительныеАдресныеСведения;
									Прервать;
							Иначе
								// дома совал полностью, но владения и корпус нет
								ТекстПроСтроения = "";
								Разделитель = "";
								Если ЗначениеЗаполнено(Корпус) Тогда
									ТекстПроСтроения  = НСтр("ru='корпус'") + " "  + Корпус;
									Разделитель = ", ";
								КонецЕсли;
								
								Если ЗначениеЗаполнено(Строение) Тогда
									ТекстПроСтроения = ТекстПроСтроения + Разделитель + НазваниеСтроения + " "  + Строение;
								КонецЕсли;
								
								ПохожиеВарианты.Добавить(НРег(НазваниеВладения) + " №" + Дом + ?(ПустаяСтрока(ТекстПроСтроения), "", ", " + ТекстПроСтроения));
								
							КонецЕсли;
							
						Иначе
							
							// номер дома совал, но тип нет
							ТекстПроДом = НРег(НазваниеВладения) + " №" + Дом;
							Если ПохожиеВарианты.Найти(ТекстПроДом) = Неопределено Тогда
								ПохожиеВарианты.Добавить(ТекстПроДом );
							КонецЕсли;
	...
	Если НЕ ДомаКорректный Тогда
		...
		НаборПредставлениеДома = Новый Массив;
		Если ЗначениеЗаполнено(НомерДомаАдрес) Тогда
			НаборПредставлениеДома.Добавить(Строка(ТипДомаАдрес) + " № "+ НомерДомаАдрес);
		КонецЕсли;
		Если ЗначениеЗаполнено(НомерКорпусаАдрес) Тогда
			НаборПредставлениеДома.Добавить(НСтр("ru='Корпус'") + " " + НомерКорпусаАдрес);
		КонецЕсли;
		Если ЗначениеЗаполнено(НомерСтроенияАдрес) Тогда
			НаборПредставлениеДома.Добавить(Строка(ТипСтроенияАдрес) + " " + НомерСтроенияАдрес);
		КонецЕсли;
		ПредставлениеДома = СтрСоединить(НаборПредставлениеДома, ", ");
		
		Подсказка = "";
		Если ПохожиеВарианты.Количество() = 1 Тогда
			Подсказка = ", " + НСтр("ru = 'но есть'") + " " + """" + ПохожиеВарианты[0]+ """.";
		ИначеЕсли ПохожиеВарианты.Количество() > 1 Тогда
			ПервоеЗначение = ПохожиеВарианты[0];
			Если ПохожиеВарианты.Количество() = 2 Тогда
				Подсказка = НСтр("ru = 'но есть ""%1"" или ""%2"".'");
				ВтороеЗначение = ПохожиеВарианты[1];
			Иначе
				Подсказка = НСтр("ru = 'но есть ""%1"" и другие варианты(%2).'");
				ВтороеЗначение = Строка(ПохожиеВарианты.Количество());
			КонецЕсли;
		...
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, ПолеСОшибкой,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" отсутствует в адресном классификаторе%2'"), ПредставлениеДома, Подсказка));
		Возврат Результат;
		
	КонецЕсли;
	
	// Проверка индекса 
	Если ЗначениеЗаполнено(ИдентификаторДополнительныхСведений) Тогда
	
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	ДополнительныеАдресныеСведения.ПочтовыйИндекс КАК ПочтовыйИндекс
			|ИЗ
		...
		Запрос.УстановитьПараметр("Идентификатор", ИдентификаторДополнительныхСведений);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Если НЕ РезультатЗапроса.Пустой() Тогда
			ДополнительныеАдресныеСведения = РезультатЗапроса.Выгрузить()[0];
			
		КонецЕсли;
	Иначе
		
		Если НЕ ДополнительныеСведения.Пустой() Тогда
			ДополнительныеАдресныеСведения = ДополнительныеСведения.Выгрузить()[0];
		КонецЕсли;
		
	КонецЕсли;
	
	ПочтовыйИндекс = Формат(ДополнительныеАдресныеСведения.ПочтовыйИндекс, "ЧГ=0");
...
Функция ДанныеУровняКорректные(Знач Адрес, Знач ИмяУровня, Знач ПредставлениеУровня, Знач АдресСтрока, Знач КакиеУровниПроверять, Знач ОшибкиПроверки)
	...
	Если КакиеУровниПроверять[ИмяУровня] <> Неопределено И ПустаяСтрока(АдресСтрока["НаименованиеУровень" + КакиеУровниПроверять[ИмяУровня]]) Тогда
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 ""%2"" отсутствует в адресном классификаторе'"), ПредставлениеУровня,
...
Процедура ФоновоеЗаданиеЗагрузкиКлассификатораАдресов(Параметры, АдресРезультата) Экспорт
	...
	ЗагрузитьКлассификаторАдресов(Параметры[0], Параметры[1],, Параметры[3]);
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	ЗаполнитьЗначенияСвойств(НомерДомаИСтроения, ОписаниеДома,
		"Идентификатор, Индекс, ОКТМО, ОКАТО, КодИФНСФЛ, КодИФНСЮЛ, КодУчасткаИФНСФЛ, КодУчасткаИФНСЮЛ");
...
//
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено, ЗагружатьИсториюАдресов = Ложь)
	...
	ЗагрузитьКлассификаторАдресов(КодыСубъектовДляЗагрузки, ОписаниеФайлов,, ЗагружатьИсториюАдресов);
	УдалитьВременныйФайл(ВременныйКаталог);
...
Процедура ЗагрузитьДополнительныеСведения(КодСубъектаРФ, КаталогФайлов, ВидыСведений, ЗагружатьИсториюИзменений)
...
Функция ЗаполнитьКодыАдреса(Результат, ИдентификаторАдресногоОбъекта, СведенияОДоме)
	...
	Если Коды <> Неопределено Тогда
		...
		Результат.ОКТМО = Формат(Результат.ОКТМО, "ЧГ=0");
		Результат.ОКАТО = Формат(Результат.ОКАТО, "ЧГ=0");
...
Функция ВыполнитьЗапросЧерезВебСервис(СтрокаЗапроса, ВремяОжидания = 120)
	...
	ТекстURL = АдресныйКлассификаторКлиентСервер.ПрефиксВерсииЗапроса() + СтрокаЗапроса;
	...
	Исключение
		Результат.Отказ = Истина;
		Результат.ПодробноеПредставлениеОшибки =Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = НСтр("ru='При выполнении запроса:'") + ТекстURL + Символы.ПС + Результат.ПодробноеПредставлениеОшибки;
	...
	Если Результат.КодСостояния = 200 Тогда
		...
		Результат.Данные = СтрокаJSONВСтруктуру(ПолученныеДанные);
	Иначе
		ОтветОтСервера = Ответ.ПолучитьТелоКакСтроку("UTF-8");
		Результат.ПодробноеПредставлениеОшибки = НСтр("ru='При получении данных из веб-сервиса возникли проблемы.'" + ОтветОтСервера);
		Результат.Отказ = Истина;
	КонецЕсли;
...
Функция СформироватьАдресПоДаннымИзКлассификатора(АдресныеСведенияДанные)
	...
	ЗаполнитьЗначенияСвойств(Адрес, АдресныеСведенияДанные, "addressType,codeKLADR,id,ifnsFLAreaCode,ifnsFLCode,ifnsULAreaCode,ifnsULCode,okato,oktmo,ZIPCode,value");
	
	Если ПустаяСтрока(Адрес.Country) Тогда
		Адрес.Вставить("Country", Строка(АдресныйКлассификаторКлиентСервер.ОсновнаяСтрана()));
	КонецЕсли;
...
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Представление = "") Экспорт
	...
	ОсновнаяСтрана = АдресныйКлассификаторКлиентСервер.ОсновнаяСтрана();
	...
	Если ТипЗнч(КонтактнаяИнформация) = Тип("Строка") Тогда
		...
		ИначеЕсли ЭтоКонтактнаяИнформацияВJSON(КонтактнаяИнформация) Тогда
			Результат = СтрокаJSONВСтруктуру(КонтактнаяИнформация);
	...
	Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес")
		 Или XDTOТип = ФабрикаXDTO.Тип("http://www.v8.1c.ru/ssl/contactinfo", "Адрес") Тогда
			...
			Иначе
				НаименованиеСтраны = АдресныйКлассификаторКлиентСервер.ОсновнаяСтрана();
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(АдресРФ) = Тип("Строка") Тогда
		...
		Если СтрЧислоВхождений(АдресРФ, ",") = 9 Тогда
			...
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[2]);
			Результат.Area     = НаименованиеИСокращение.Наименование;
			Результат.AreaType = НаименованиеИСокращение.Сокращение;
			
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[3]);
			Результат.District     = НаименованиеИСокращение.Наименование;
			Результат.DistrictType = НаименованиеИСокращение.Сокращение;
			
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[4]);
			Результат.City         = НаименованиеИСокращение.Наименование;
			Результат.CityType     = НаименованиеИСокращение.Сокращение;
			
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[5]);
			Результат.Locality     = НаименованиеИСокращение.Наименование;
			Результат.LocalityType = НаименованиеИСокращение.Сокращение;
			
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[6]);
			Результат.Street       = НаименованиеИСокращение.Наименование;
			Результат.StreetType   = НаименованиеИСокращение.Сокращение;
			
			НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[7]);
			...
			Если ЗначениеЗаполнено(ЧастиАдреса[8]) Тогда
				НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[8]);
				Результат.Buildings.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(
				НаименованиеИСокращение.Сокращение, НаименованиеИСокращение.Наименование));
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЧастиАдреса[9]) Тогда
				НаименованиеИСокращение = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЧастиАдреса[9]);
				Результат.Apartments.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(
				НаименованиеИСокращение.Сокращение, НаименованиеИСокращение.Наименование));
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		РайонАдреса = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(РайонАдреса(АдресРФ));
		Результат.District     = Строка(РайонАдреса.Наименование);
		Результат.DistrictType = Строка(РайонАдреса.Сокращение);
		
		Город = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(АдресРФ.Город);
		Результат.City     = Строка(Город.Наименование);
		Результат.CityType = Строка(Город.Сокращение);
		
		НаселПункт = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(АдресРФ.НаселПункт);
		Результат.Locality     = Строка(НаселПункт.Наименование);
		Результат.LocalityType = Строка(НаселПункт.Сокращение);
		
		Улица = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(АдресРФ.Улица);
		Результат.Street     = Строка(Улица.Наименование);
		Результат.StreetType = Строка(Улица.Сокращение);
		
		ВнутригРайон = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(АдресРФ.ВнутригРайон);
		...
		Если ЗначениеЗаполнено(ЗначениеДополнительныхЭлементов.ДополнительныйЭлемент) Тогда
			ДополнительныйЭлемент = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЗначениеДополнительныхЭлементов.ДополнительныйЭлемент);
		...
		Если ЗначениеЗаполнено(ЗначениеДополнительныхЭлементов.ПодчиненныйЭлемент) Тогда
			ПодчиненныйЭлемент = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(ЗначениеДополнительныхЭлементов.ПодчиненныйЭлемент);
