...
Процедура СформироватьПрограммныйИнтерфейс() Экспорт
	...
	Параметр = СтруктураПрограммногоИнтерфейса;
...
Процедура ПрочитатьПрограммныйИнтерфейсМодуля(Знач ПрограммныйИнтерфейс)
	...
	Для Итератор = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если СтрНайти(Строка, "#Область УстаревшиеПроцедурыИФункции") > 0
			...
			Пока СтрНайти(Строка, "#КонецОбласти") = 0 Цикл
				Итератор = Итератор + 1;
				Строка = ТекстовыйДокумент.ПолучитьСтроку(Итератор);
			КонецЦикла;
		КонецЕсли;
...
Процедура ПрочитатьКомментарийИМетод(Строка, ТекстовыйДокумент, Итератор, Рекурсия = Ложь)
	...
	Если СтрНайти(Строка, "Параметры:") > 0 И СтрРазделить(Строка, " ", Ложь).Количество() = 2 Тогда
		ЧтениеПараметров = Истина;
		Возврат;
	ИначеЕсли СтрНайти(Строка, "Возвращаемое значение:") > 0 Тогда
		ЧтениеПараметров           = Ложь;
		ЧтениеВозвращаемыхЗначений = Истина;
		Возврат;
	ИначеЕсли СтрНайти(Строка, "// Пример:") > 0 И СтрРазделить(Строка, " ", Ложь).Количество() = 2 Тогда
		ЧтениеПараметров           = Ложь;
	...
	// Логирование.
	Если СтрНайти(СтруктураМетода.ОписаниеМетода, "Только для внутреннего использования") > 0
...
Процедура СохранитьОписаниеВHtml()
	...
	Подробности = "<p class=""Paragraph0c"">"
		...
		+ "<p class=""Paragraph0c"">" + НСтр("ru = 'При выпуске новых версий библиотеки в этих процедурах и функциях обеспечивается обратная совместимость, что
			|не требует от прикладных разработчиков пересматривать свой код и адаптировать объекты метаданных своих
		...
		+ "<p class=""Paragraph0c"">" + НСтр("ru = 'Подробнее о программном интерфейсе и переопределяемых модулях см. в 
			|%1,
			|раздел «Разработка и использование библиотек».'") + "</p>";
...
Процедура ЗаполнитьСоответствиеОбъектовПодсистемам(Соответствие, КоллекцияПодсистем = Неопределено)
	...
	Для Каждого ЭлементПодсистема Из КоллекцияПодсистем Цикл
		...
		Если ЭлементПодсистема.Подсистемы.Количество() > 0 Тогда
