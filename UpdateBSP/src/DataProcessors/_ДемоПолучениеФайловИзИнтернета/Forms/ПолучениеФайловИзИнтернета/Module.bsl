...
Процедура АдресВоВременномХранилищеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ПолучитьФайл(АдресВоВременномХранилище, "demo.txt");
...
Процедура ЗагрузитьФайл(Команда)
	...
	Если КудаСохранять = 0 Тогда
		...
		Если Не ПустаяСтрока(ПустаяСтрока(ПутьНаКлиенте)) Тогда 
	...
	ИначеЕсли КудаСохранять = 1 Тогда
		...
		Если Не ПустаяСтрока(ПустаяСтрока(ПутьНаСервере)) Тогда 
...
Процедура ВыбратьКаталог(Знач Оповещение)
	...
	Диалог.Показать(Новый ОписаниеОповещения("ВыбратьКаталогЗавершение", ЭтотОбъект, Новый Структура("Диалог, Оповещение", Диалог, Оповещение)));
