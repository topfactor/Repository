...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПолученоСообщениеДляСопоставленияДанных Тогда
		...
		Иначе
			ПоместитьСообщениеДляСопоставленияДанныхВХранилище(Объект.УзелИнформационнойБазы, Объект.ИмяФайлаСообщенияОбмена);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	ОбщегоНазначенияКлиент.ОткрытьПроводник(ИмяКаталога);
...
Процедура ПриИзмененииФлагаЗакончитьСопоставлениеДанных()
	...
	Элементы.ЗагрузитьСообщениеПослеСопоставления.Видимость = ЗакончитьСопоставлениеДанных;
...
Процедура ОбновитьИнформациюСопоставленияПоСтрокеНаСервере(Отказ, КлючиСтрок)
	...
	ЕстьНеСопоставленнаяНСИ = НЕ ВсеДанныеСопоставлены И Обработки.ПомощникИнтерактивногоОбменаДанными.ЕстьНеСопоставленнаяНСИ(ОбработкаОбъект.ТаблицаИнформацииСтатистики());
...
Процедура ПриИзмененииВидаТранспортаСообщенийОбмена(Инициализация = Ложь)
	...
	ИспользоватьПрогрессБар = Не ОбменЧерезПодключениеККорреспонденту И Не ОбменМеждуПриложениямиВСервисе;
...
&НаСервереБезКонтекста
Процедура ПоместитьСообщениеДляСопоставленияДанныхВХранилище(Корреспондент, ИмяФайлаСообщенияОбмена)
	
	Отбор = Новый Структура("УзелИнформационнойБазы", Корреспондент);
	...
	Если ЗначениеЗаполнено(ОбщиеНастройки.СообщениеДляСопоставленияДанных) Тогда
		...
		Если Файл.Существует() Тогда
			...
			ИмяФайлаСообщенияВХранилище = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(
				ОбменДаннымиПовтИсп.КаталогВременногоХранилищаФайлов(), Файл.Имя);
			ПереместитьФайл(ИмяФайлаСообщенияОбмена, ИмяФайлаСообщенияВХранилище);
				
			ОбменДаннымиСервер.ПоместитьФайлВХранилище(ИмяФайлаСообщенияВХранилище,
				Новый УникальныйИдентификатор(ОбщиеНастройки.СообщениеДляСопоставленияДанных));
...
Процедура ПриОжиданииРегистрацииДанных()
	...
	Если ПродолжитьОжидание Тогда
		ДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжиданияРегистрацииДанных);
...
Процедура ПриНачалеРегистрацииДанныхНаСервере(ПродолжитьОжидание)
	...
	Обработки.ПомощникИнтерактивногоОбменаДанными.ПриНачалеРегистрацииДанных(НастройкиРегистрации,
...
Процедура ПриОжиданииРегистрацииДанныхНаСервере(ПараметрыОбработчика, ПродолжитьОжидание)
	...
	ПродолжитьОжидание = Ложь;
	Обработки.ПомощникИнтерактивногоОбменаДанными.ПриОжиданииРегистрацииДанных(ПараметрыОбработчика, ПродолжитьОжидание);
...
Процедура ПриЗавершенииРегистрацииДанныхНаСервере(ПараметрыОбработчика, ДанныеЗарегистрированы, СообщениеОбОшибке)
	...
	Обработки.ПомощникИнтерактивногоОбменаДанными.ПриЗавершенииРегистрацииДанных(
...
Процедура ПриОжиданииВыгрузкиДанных()
	...
	Если ПродолжитьОжидание Тогда
		ДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжиданияВыгрузкиДанных);
...
Процедура ПриНачалеВыгрузкиДанныхНаСервере(ПродолжитьОжидание)
	...
	МодульПомощникИнтерактивногоОбмена = ОбщегоНазначения.ОбщийМодуль(
		"Обработки.ПомощникИнтерактивногоОбменаДаннымиВМоделиСервиса");
