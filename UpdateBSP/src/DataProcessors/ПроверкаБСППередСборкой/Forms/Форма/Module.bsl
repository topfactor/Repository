...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(НСтр("ru = 'Некорректны вызовы процедур и функций общего модуля или модуля менеджера из него самого, содержащие имя модуля.'"), 
...
#Если Не ВебКлиент Тогда
	...
	ОбщегоНазначенияКлиентСервер.ЗапуститьПрограмму(КомандаЗапуска);
...
&НаСервере
Процедура ДобавитьПроверку(ИмяПроверки, ИмяПроцедурыПроверки)
	...
	Для Каждого ТипМетаданныхВСписке Из СписокТиповМетаданных Цикл
		...
		Для Каждого ОбъектМетаданныхЗаданногоТипа Из Метаданные[ТипМетаданных] Цикл
			Если ТипМетаданных = "ПараметрыСеанса"
				...
				Или СтрНайти(ОбъектМетаданныхЗаданногоТипа.Имя, "КлючиДоступаКРегистру_ДемоОстаткиТоваровВМестахХранения") > 0 Тогда
				Продолжить;
			КонецЕсли;
...
Функция Подключаемый_ПроверитьКорректностьКомментированияДемоПримеров()
	...
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		...
		Если СтрНайти(ФайлМодуля.ПолноеИмя, "ПереходНаВерсию231") > 0
			Или СтрНайти(ФайлМодуля.ПолноеИмя, "ПереходНаВерсию301") > 0
...
Функция Подключаемый_ПроверитьВызовыОбщихМодулейИзНихСамих()
	...
	ПроверитьВызовыОбщихМодулейИзНихСамих(СписокОшибочныхОбъектов);
	
	ПредставлениеОшибки = НСтр("ru = 'Название модуля в его коде'");
	ОписаниеИсправления = НСтр("ru = 'Возможен некорректный вызов процедуры или функции общего модуля или модуля менеджера из него самого.
...
Функция Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()
	...
	ВидыДоступаПоСсылкам = СвойстваВидовДоступа.ПоСсылкам;
...
Функция Подключаемый_НайтиНерекомендуемыеНастройкиРолей()
	...
	Если ЗначениеЗаполнено(Результат.НекорректнаяНастройкаПрав) Тогда
		Пояснение = НСтр("ru = '1.1. См. пункт 1 стандарта ""Проверка прав доступа"" https://its.1c.ru/db/v8std#content:2149184386:hdoc
...
&НаСервере
Функция ПроверитьВызовыОбщихМодулейИзНихСамих(СписокОшибочныхОбъектов)
	
	// Объекты, не относящиеся к подсистемам БСП и не являющиеся демо-объектами.
	ЗаполнитьПринадлежностьОбъектовПодсистемам(Метаданные.Подсистемы.АдминистрированиеСервиса);
	ЗаполнитьПринадлежностьОбъектовПодсистемам(Метаданные.Подсистемы.ИнтернетПоддержкаПользователей);
	ЗаполнитьПринадлежностьОбъектовПодсистемам(Метаданные.Подсистемы.ТехнологияСервиса);
	...
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		...
		ПодсистемыОбъекта = СоответствиеОбъектовИПодсистем[ПолноеИмяСтруктура.ПолноеИмяОбъекта];
		...
		Если СтрНайти(ПолноеИмяСтруктура.ПолноеИмяОбъекта, "ОбщийМодуль") = 0 Тогда
			Если СтрНайти(ПолноеИмяСтруктура.ПолноеИмяМодуля, "МодульМенеджера") = 0 Тогда
				Продолжить;
			КонецЕсли;
			ПолноеИмяОбъектаЧастями = СтрРазделить(ПолноеИмяСтруктура.ПолноеИмяОбъекта, ".");
			ТипОбъекта   = ПолноеИмяОбъектаЧастями[0];
			ИмяМенеджера = ИмяМенеджераОбъектаПоТипу(ТипОбъекта);
			ПолноеИмяОбъектаЧастями.Установить(0, ИмяМенеджера);
			
			
			ИмяОбщегоМодуля = СтрСоединить(ПолноеИмяОбъектаЧастями, ".") + ".";
		Иначе
			ИмяОбщегоМодуля = ПолноеИмяСтруктура.ИмяОбъекта + ".";
		КонецЕсли;
...
Функция ИмяПодсистемыЗаданоНекорректно(УсловныйВызов, СписокИспользуемыхПодсистем)
	...
	Для Каждого ИспользуемаяПодсистема Из СписокИспользуемыхПодсистем Цикл
		...
		СтрокаУсловногоВызова = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ОбщегоНазначения.ПодсистемаСуществует(""%1"")", ИспользуемаяПодсистема);
		СтрокаУсловногоВызова2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ОбщегоНазначенияКлиент.ПодсистемаСуществует(""%1"")", ИспользуемаяПодсистема);
...
Процедура ПроверитьПраваНаРеквизиты(Ошибки, ОбъектXDTO, ИмяРоли)
	...
	НезависимыеПраваПодчиненныхОбъектов = СвойствоРоли(ТекстОшибок.НекорректнаяНастройкаПрав,
		ОбъектXDTO, "independentRightsOfChildObjects", Ложь);
