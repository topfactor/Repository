...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМеста(
	...
	Попытка
		...
		Если ВерсияБСП301 Тогда
			...
			МодульОбменДаннымиСервер.ЗавершитьСозданиеНачальногоОбраза(АвтономноеРабочееМесто);
		КонецЕсли;
...
Функция СоздатьНовоеАвтономноеРабочееМесто(Настройки)
	...
	Если ВерсияБСП301 Тогда
		МодульОбменДаннымиСервер = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСервер");
		МодульОбменДаннымиСервер.ЗавершитьНастройкуСинхронизацииДанных(АвтономноеРабочееМестоОбъект.Ссылка);
	КонецЕсли;
