...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредлагатьАвтоподбор  = КлассификаторДоступен Или ЕстьКлассификатор;
	Если Не ПредлагатьАвтоподбор Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.Переместить(Элементы.ГруппаКомандаДобавить, Элементы.ПанельКомандМобильныйКлиент);
		Элементы.Переместить(Элементы.ФормаКомандаОК, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ПроверитьЗаполнениеАдреса, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ИсторияИзменений, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.АдресНаКартеЯндексКартах, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ПоказатьНаКартеGoogle, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.МуниципальноеДеление, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.АдминистративноТерриториальноеДеление, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.АдресВСвободнойФорме, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ФормаОчиститьАдрес, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ЗаполнитьПоПочтовомуИндексу, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ЗагрузитьКлассификатор, Элементы.ФормаКоманднаяПанель);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НаселенныйПункт", "Заголовок", НСтр("ru ='Город'"));
...
Процедура ФормаВыбораУлицПоИндексу(Знач Текст)
	...
	Модифицированность = Истина;
	Индекс = Текст;
	Если ПредлагатьАвтоподбор Тогда
		ПараметрыФормы = Новый Структура;
...
Функция ДанныеБылиИзменены(ЗначениеПолейДо, ЗначениеПолейПосле)
	...
	Если ЗначениеЗаполнено(ЗначениеПолейДо)И ЗначениеЗаполнено(ЗначениеПолейПосле) Тогда
		...
		ЗначенияДо    = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(ЗначениеПолейДо);
		ЗначенияПосле = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(ЗначениеПолейПосле);
...
Процедура ЗаполнитьПоляДомовИСтроений(Знач ВыбранноеЗначение)
	...
	НаселенныйПунктДетально.id      = ВыбранноеЗначение.Идентификатор;
