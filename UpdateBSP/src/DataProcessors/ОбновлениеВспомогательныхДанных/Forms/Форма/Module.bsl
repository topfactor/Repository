...
Процедура ОбновитьДоступностьЭлементов(Форма)
	...
	// СтандартныеПодсистемы КонтрольВеденияУчета
	Элементы.ПравилаПроверкиУчета.Доступность = Форма.ОбновитьНеразделенныеДанные;
...
Функция УказаннаяОбластьДанныхЗаполнена(Знач ОбластьДанных)
	...
	ЭлементБлокировки = Блокировка.Добавить(РегистрОбластиДанных);
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Запрос.Текст, РегистрОбластиДанных);
