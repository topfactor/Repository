...
Процедура ДобавитьПрефиксУдалить(ФайлСправочника, ИмяОбъекта, ИмяРеквизита, ТипОбъекта)
	...
	ФлагОшибки = Ложь;
	ИмяСправочника = СтрЗаменить(ФайлСправочника.Имя, "Catalog.", "Справочник.");
	ПутьКФайлуОбъекта = ФайлСправочника.ПолноеИмя;
	ПутьКФайлуОпределяемогоТипа = ФайлСправочника.ПолноеИмя;
	...
	ДокументDOMОпределяемогоТипа = ПрочитатьДокументDOM(ПутьКФайлуОпределяемогоТипа);
	Разыменователь = Новый РазыменовательПространствИменDOM(СоответствиеПространстваИмен());
	СоответствиеВыражений = СоответствиеВыражений();
	Выражение = СоответствиеВыражений.Получить("FunctionalOption");
	СоставОпределяемогоТипа = ВычислитьВыражениеXPath(Выражение , ДокументDOMОпределяемогоТипа, Разыменователь);
	СоответствиеТипов = СоставОпределяемогоТипаВСоответствие(СоставОпределяемогоТипа, ФайлСправочника);
...
Процедура ДополнитьСоставОпределяемогоТипаИзРеквизита(ФайлСправочника, ИмяОбъекта, ИмяРеквизита, ТипОбъекта, Параметры)
	...
	Для Каждого РеквизитСправочника Из СписокРеквизитовИТЧ Цикл
		...
		ИначеЕсли РеквизитСправочника.ИмяУзла = "Attribute" Тогда
			...
			ИначеЕсли СписокРеквизитов.ПервыйДочерний.ТекстовоеСодержимое = "ТипХраненияФайла" Тогда
				ТипХраненияФайла = СписокРеквизитов.РодительскийУзел;
	...
	Если НЕ ЕстьРеквизитДатаЗаема Тогда
		...
		УзелV8lang = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелV8Item, "v8:lang", "ru");
		УзелV8lang = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелV8Item, "v8:content", НСтр("ru = 'Дата заема'"));
		...
		УзелDateQualifiers = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелType, "v8:DateQualifiers", "");
	...
	Если НЕ ЕстьРеквизитХранитьВерсии Тогда
		...
		УзелV8Item = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелSynonym, "v8:item", "");
		УзелV8lang = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелV8Item, "v8:lang", "ru");
		УзелV8lang = ДобавитьДочернийУзел(ДокументDOMОбъекта, УзелV8Item, "v8:content", НСтр("ru = 'Хранить версии'"));
...
Функция СоставОпределяемогоТипаВСоответствие(СоставОпределяемогоТипа, ИмяОпределяемогоТипа)
	...
	СоставОпределяемогоТипа = СоставОпределяемогоТипа.ПолучитьСледующий();
...
Процедура ПодключаемыеКоманды_ПриЧтенииОсновныхНастроек(Параметры)
	...
	Параметры.Вставить("ЕстьЗаполнениеОбъектов", ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗаполнениеОбъектов"));
	...
	Параметры.Вставить("НовыеОбъектыСКомандами", ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ОбъектыСКомандами));
...
Функция ЗагрузитьКонфигурациюИзXML(Параметры)
	...
	ЗаписьТекста.Записать(ОбщегоНазначенияКлиентСервер.БезопаснаяСтрокаКоманды(КомандаЗапуска));
	ЗаписьТекста.Закрыть();
