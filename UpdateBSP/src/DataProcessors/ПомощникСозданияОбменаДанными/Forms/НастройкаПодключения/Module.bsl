...
Процедура ПриНачалеСохраненияНастроекПодключенияНаСервере(ПродолжитьОжидание)
	...
	Если ЭтоОбменСПриложениемВСервисе Тогда
		...
		Если НастройкаXDTO Тогда
			...
			НастройкиПодключения.НастройкиXDTOКорреспондента.Вставить("ПоддерживаемыеОбъекты", ПоддерживаемыеОбъектыФормата);
...
Процедура ИнициализироватьРеквизитыФормы()
	...
	ИспользоватьПрефиксыДляНастройкиОбмена = Не ОбменДаннымиПовтИсп.ЭтоПланОбменаXDTO(ИмяПланаОбмена)
		Или Не ОбменДаннымиXDTOСервер.ПоддерживаетсяВерсияСИдентификаторомОбменаДанными(ПланыОбмена[ИмяПланаОбмена].ПустаяСсылка());
