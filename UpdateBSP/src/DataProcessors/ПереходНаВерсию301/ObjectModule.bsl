...
Функция ДобавитьЭлементВКонец(ЭлементМодуля, ДобавлятьОтступы = Истина)
	...
	Если ДобавлятьОтступы И Не ЕстьОтступВКонце Тогда
		ЭлементМодуля.Содержимое.Добавить("");
	КонецЕсли;
...
Функция ДобавитьЭлементВНачало(ЭлементМодуля, ДобавлятьОтступы = Истина)
	...
	Если ДобавлятьОтступы И Не ЕстьОтступВНачале Тогда
		ЭлементМодуля.Содержимое.Вставить(0, "");
	КонецЕсли;
...
Процедура ДобавитьСтрокуВКонец(ЭлементМодуля, Строка)
	...
	ПозицияВставки = ЭлементМодуля.Содержимое.Количество();
	
	ЕстьОтступВКонце = ЭлементМодуля.Содержимое.Количество() > 0
		И (ТипЗнч(ЭлементМодуля.Содержимое[ЭлементМодуля.Содержимое.ВГраница()]) <> Тип("Структура")
			И ПустаяСтрока(ЭлементМодуля.Содержимое[ЭлементМодуля.Содержимое.ВГраница()]));
	
	Если ЕстьОтступВКонце И ЭлементМодуля.Содержимое.Количество() > 1 Тогда
		Если ТипЗнч(ЭлементМодуля.Содержимое[ЭлементМодуля.Содержимое.ВГраница() - 1]) = Тип("Структура") Тогда
			ЭлементМодуля.Содержимое.Добавить("");
		КонецЕсли;
		ПозицияВставки = ЭлементМодуля.Содержимое.ВГраница();
	КонецЕсли;
...
Функция ЗаменяемыеПсевдонимыТаблиц(ИмяОсновнойТаблицы, Знач ПрисоединяемыеТаблицы)
	...
	Для Каждого Соединение Из СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Источники[0].Соединения Цикл
		...
		Если Не СтрНачинаетсяС(НРег(ИмяТаблицы), НРег("Регистр")) Тогда
