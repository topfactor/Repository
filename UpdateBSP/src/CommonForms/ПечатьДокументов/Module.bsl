			...
			Для Каждого ФайлОфисногоДокумента Из ФайлыОфисныхДокументов Цикл
				...
				Если ЗаписьZipФайла <> Неопределено Тогда 
					ПолноеИмяФайла = УникальноеИмяФайла(ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяВременнойПапки) 
						+ ФайлОфисногоДокумента.Значение);
				...
				Иначе
					...
					ОписаниеФайла.Вставить("Представление", ФайлОфисногоДокумента.Значение);
...
Процедура СохранитьПечатнуюФормуВФайл()
	...
	Для Каждого ФайлДляЗаписи Из ФайлыВоВременномХранилище Цикл
		ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ФайлДляЗаписи.Представление, "");
		Расширение = НастройкиФорматаСохранения.Расширение;
		Если ФайлДляЗаписи.Свойство("ЭтоОфисныйДокумент") Тогда
			Расширение = "docx";
		КонецЕсли;
		ПолучаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(ИмяФайла + "." + Расширение, ФайлДляЗаписи.АдресВоВременномХранилище));
	КонецЦикла;
	
	ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
	ДиалогВыбора.Заголовок = НСтр("ru ='Выбор папки для сохранения сформированного документа'");
	Оповещение = Новый ОписаниеОповещения("СохранитьПечатнуюФормуВФайлПослеПолученияФайлов", ЭтотОбъект);
	НачатьПолучениеФайлов(Оповещение, ПолучаемыеФайлы, ДиалогВыбора, ЛОЖЬ);
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьПечатнуюФормуВФайлПослеПолученияФайлов(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Файл Из ПолученныеФайлы Цикл
		ОбщегоНазначенияКлиент.ОткрытьФайлВПрограммеПросмотра(Файл.Имя);
	КонецЦикла;
