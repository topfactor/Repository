...
Процедура ОбновитьВидимостьНаСервере()
	...
	Если ВерсияБСП301 Тогда
		МодульНастройкиТранспорта = РегистрыСведений["НастройкиТранспортаОбменаДанными"];
	Иначе
		МодульНастройкиТранспорта = РегистрыСведений["НастройкиТранспортаОбмена"];
	КонецЕсли;
	НастройкиТранспортаWS = МодульНастройкиТранспорта.НастройкиТранспортаWS(ПриложениеВСервисе);
...
Процедура НастроитьПодключениеКСервису(НастройкаАвтоматическойСинхронизации = Ложь)
	...
	Иначе
		ОбменДаннымиКлиент.ОткрытьФормуЗаписиРегистраСведенийПоОтбору(Отбор,
			ЗначенияЗаполнения, "НастройкиТранспортаОбмена", ЭтотОбъект, "НастройкаПодключенияКСервису", ПараметрыФормы);
	КонецЕсли;
