...
Процедура СсылкаНаСтраницуПрограммыНажатие(Элемент)
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(АдресПрограммыДляОткрытияМакета);
КонецПроцедуры
...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	ВидимостьСсылкиНаСтраницуПрограммы = (ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Или ТипФайла <> "mxl") И ТипФайла <> "docx";
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Иначе
		...
		КонецЕсли;
		ОбщегоНазначенияКлиент.ОткрытьФайлВПрограммеПросмотра(ПутьКФайлуМакета);
	КонецЕсли;
