...
Процедура ДобавитьЗапись(СтруктураЗаписи) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		НачатьТранзакцию();
		Попытка
			ОбменДаннымиСервер.ДобавитьЗаписьВРегистрСведений(Новый Структура("Корреспондент", СтруктураЗаписи.Корреспондент),
...
Функция НастройкиТранспортаWS(Корреспондент, ПараметрыАутентификации = Неопределено) Экспорт
	...
	WSПароль = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(Корреспондент, "WSПароль", Истина);
...
Функция НастройкиТранспортаОбмена(Корреспондент, ВидТранспортаОбмена)
	...
	УстановитьПривилегированныйРежим(Истина);
	Пароли = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(Корреспондент, СписокПаролей, Истина);
	УстановитьПривилегированныйРежим(Ложь);
