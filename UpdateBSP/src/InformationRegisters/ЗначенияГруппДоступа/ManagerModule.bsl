		...
		Если ЕстьТекущиеИзменения
		   И УправлениеДоступомСлужебный.ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
			...
			СтандартныеПодсистемыСервер.УстановитьПараметрРаботыРасширения(
				"УправлениеДоступом.ЗначенияГруппДоступаВерсия", Неопределено, Истина);
			
			// Планирование обновления доступа.
			ПустыеСсылкиТиповГруппИЗначений = УправлениеДоступомСлужебныйПовтИсп.ПустыеСсылкиТиповГруппИЗначений();
			...
			Для Каждого Строка Из Данные.ИзмененияСоставаСтрок Цикл
				...
				НоваяСтрока.ТипЗначенийДоступа = ПустыеСсылкиТиповГруппИЗначений.Получить(ТипЗнч(Строка.ЗначениеДоступа));
			КонецЦикла;
