...
Функция ПодготовкаДанных(ДокументОтбор, ДвиженияДокумента, ТаблицаРегистров, ДополнительнаяНумерация = Ложь)
	...
	Для Каждого СтрокаТаблицыРегистров Из ТаблицаРегистров Цикл
		...
		ПолеРегистратора       = СокрЛП(СтрокаТаблицыРегистров.ПолеРегистратора);
		
		ДополнительнаяИнформация = Новый Структура;
		ДополнительнаяИнформация.Вставить("ИмяРегистра",             ИмяРегистра);
		ДополнительнаяИнформация.Вставить("ВидРегистра",             ВидРегистра);
		ДополнительнаяИнформация.Вставить("ЛокализацияВидРегистра",  ЛокализацияВидРегистра);
		ДополнительнаяИнформация.Вставить("ПредставлениеРегистра",   ПредставлениеРегистра);
		ДополнительнаяИнформация.Вставить("ПолеРегистратора",        ПолеРегистратора);
		ДополнительнаяИнформация.Вставить("ДополнительнаяНумерация", ДополнительнаяНумерация);
		...
		Для Каждого ДвижениеДокумента Из ДвиженияДокумента Цикл
			...
			Если ВРег(СокрЛП(ДвижениеДокумента.Имя)) = ВРег(ИмяРегистра) Тогда
				Если ВидРегистра = "Накопления" Или ВидРегистра = "Сведений" Тогда
					ПодготовкаДанныхПоРегистрамНакопленияСведений(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация);
				ИначеЕсли ВидРегистра = "Бухгалтерии" Тогда
					ПодготовкаДанныхПоРегистрамБухгалтерии(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация);
				ИначеЕсли ВидРегистра = "Расчета" Тогда
					ПодготовкаДанныхПоРегистрамРасчета(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация);
				КонецЕсли;
			КонецЕсли;
	...
	ОтчетОДвиженияхДокументаПереопределяемый.ПриПодготовкеНабораДанных(ДокументОтбор, ОсновнаяИнформация);
	
	Возврат ОсновнаяИнформация;
	
КонецФункции

Процедура ПодготовкаДанныхПоРегистрамНакопленияСведений(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация)
	...
	ИнформацияПоРегистру.Вставить("ИмяРегистра",            ДополнительнаяИнформация.ИмяРегистра);
	ИнформацияПоРегистру.Вставить("ВидРегистра",            ДополнительнаяИнформация.ВидРегистра);
	ИнформацияПоРегистру.Вставить("ЛокализацияВидРегистра", ДополнительнаяИнформация.ЛокализацияВидРегистра);
	ИнформацияПоРегистру.Вставить("ПредставлениеРегистра",  ДополнительнаяИнформация.ПредставлениеРегистра);
	
	ДополнительнаяНумерация = ДополнительнаяИнформация.ДополнительнаяНумерация;
	...
	ПоляВыбора = "";
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.СтандартныеРеквизиты, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Измерения, ДвижениеДокумента, "Измерения");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Ресурсы, ДвижениеДокумента, "Ресурсы");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Реквизиты, ДвижениеДокумента, "Реквизиты");
	...
	ПолноеИмяРегистра = "Регистр" + ДополнительнаяИнформация.ВидРегистра + "_" + ДополнительнаяИнформация.ИмяРегистра;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеЗапроса", ДополнительнаяИнформация.ПолеРегистратора + " = &ДокументВладелец");
...
Процедура ПодготовкаДанныхПоРегистрамБухгалтерии(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация)
	...
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Реквизиты, ДвижениеДокумента, "Реквизиты");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Измерения, ДвижениеДокумента, "Измерения");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.ИзмеренияДт, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.ИзмеренияКт, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Ресурсы, ДвижениеДокумента, "Ресурсы");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.РесурсыДт, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.РесурсыКт, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Субконто, ДвижениеДокумента, "Субконто");
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.СубконтоДт, ДвижениеДокумента);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.СубконтоКт, ДвижениеДокумента);
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяРегистра", ПолноеИмяРегистра);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Поля", ПоляВыбора);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекущаяТаблица", "РегистрБухгалтерии." + ДополнительнаяИнформация.ИмяРегистра
		+ ".ДвиженияССубконто(, , " + ДополнительнаяИнформация.ПолеРегистратора + " = &ДокументВладелец)");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеЗапроса", ДополнительнаяИнформация.ПолеРегистратора + " = &ДокументВладелец");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеКомпоновки", """" + ПолноеИмяРегистра + """ В (&СписокРегистров)");
...
Процедура ПодготовкаДанныхПоРегистрамРасчета(ДвижениеДокумента, ОсновнаяИнформация, ДополнительнаяИнформация)
...
Функция ЗаполнитьСхемуКомпоновкиДанных_Горизонталь(ДокументОтбор, НастройкиКД)
	...
	Иначе
		...
		ПодготовленныеДанные = ПодготовкаДанных(ДокументОтбор, ДвиженияДокумента, ТаблицаРегистров);
...
Функция ЗаполнитьСхемуКомпоновкиДанных_Вертикаль(ДокументОтбор, НастройкиКД, ДополнительнаяНумерация = Ложь)
	...
	Иначе
		ТаблицаРегистров     = ТаблицаИспользуемыхРегистров(МассивИспользуемыхРегистров);
		ПодготовленныеДанные = ПодготовкаДанных(ДокументОтбор, ДвиженияДокумента, ТаблицаРегистров, ДополнительнаяНумерация);
...
Процедура ДобавитьПоля(ПоляВыбора, СтруктураПолей, ДвижениеДокумента, ИмяКоллекции = "")
	...
	Для Каждого Поле Из СтруктураПолей Цикл
		...
		Иначе
			Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ДвижениеДокумента[ИмяКоллекции][Поле.Ключ]) Тогда
				ПоляВыбора = ПоляВыбора + ?(ЗначениеЗаполнено(ПоляВыбора), ", ", "") + Поле.Ключ;
			КонецЕсли;
		КонецЕсли;
...
Функция МассивИспользуемыхРегистров(Регистратор, Движения)
	...
	ОтчетОДвиженияхДокументаПереопределяемый.ПриОпределенииРегистровСДвижениями(Регистратор, Результат);
...
Функция ТаблицаИспользуемыхРегистров(МассивИспользуемыхРегистров)
	...
	КолонкиТаблицыРегистров.Добавить("ПолеРегистратора",              Новый ОписаниеТипов("Строка"));
	
	Для Каждого ИспользуемыйРегистр Из МассивИспользуемыхРегистров Цикл
		
		ПолеРегистратора    = ИспользуемыйРегистр.Значение;
		ИспользуемыйРегистр = ИспользуемыйРегистр.Ключ;
