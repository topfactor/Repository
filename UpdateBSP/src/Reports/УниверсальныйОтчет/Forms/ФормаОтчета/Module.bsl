...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьИсточникДанныхТекущегоВарианта(Параметры.Ссылка);
...
#Если ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
		Если Тип(Результат) = Тип("СхемаКомпоновкиДанных") Тогда
			...
			НачатьУдалениеФайлов(Новый ОписаниеОповещения, Путь);
...
Процедура Подключаемый_ИмяТаблицы_ПриИзменении(Элемент)
	...
	Если НастройкиОтчета.Свойство("Период") Тогда 
		Параметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Период");
		Параметр.Значение = НастройкиОтчета.Период;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ИмяОбъектаМетаданных_ПриИзменении(Элемент)
	
	ИдентификаторЭлемента = Прав(Элемент.Имя, 32);
	
	ПользовательскаяНастройкаКД = НайтиПользовательскуюНастройкуЭлемента(ИдентификаторЭлемента);
	
	Параметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИмяОбъектаМетаданных");
	Если Параметр.Значение = ПользовательскаяНастройкаКД.Значение Тогда
		Возврат;
	КонецЕсли;
	Параметр.Значение = ПользовательскаяНастройкаКД.Значение;
	
	Параметр = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИсточникДанных");
	Параметр.Значение = "";
...
Процедура Подключаемый_СтандартныйПериод_НачалоПериода_ПриИзменении(Элемент)
	...
	НастройкиОтчета.Вставить("Период", Значение);
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступность(ОбновитьКоманды = Истина)
	...
	Элементы.ЗагрузитьСхемуИзФайла.Видимость = Не ОбщегоНазначения.РазделениеВключено();
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее) Экспорт
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СвязьСКомпоновщиком" Тогда
			...
			ИначеЕсли ИмяПараметра = "ПараметрыДанных.ИмяОбъектаМетаданных" Тогда
				ПолеВвода.УстановитьДействие("ПриИзменении", "Подключаемый_ИмяОбъектаМетаданных_ПриИзменении");
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СписокСПодбором" Тогда
			...
			Если Не СвойстваНастройки.ВыводитьФлажок Или Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
				...
				ГруппаЗаголовокТаблицы.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
			...
			ГруппаСОтступом.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	...
	Если ИмяПараметра = "ПараметрыДанных.ТипОбъектаМетаданных"
		Или ИмяПараметра = "ПараметрыДанных.ИмяОбъектаМетаданных"
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Для Каждого СвойстваНастройки Из ДобавленныеСтандартныеПериоды Цикл
		...
		Если Не НастройкиОтчета.Свойство("Период") Тогда 
			НастройкиОтчета.Вставить("Период", СвойстваНастройки.Значение);
		КонецЕсли;
		ЭтотОбъект[Дополнительно.ЗначениеИмя] = НастройкиОтчета.Период;
...
// Устанавливает параметр "ИсточникДанных" настроек текущего варианта отчета
//
// Параметры:
//  Вариант - СправочникСсылка.ВариантыОтчетов - хранилище настроек варианта отчета.
//
&НаСервереБезКонтекста
Процедура УстановитьИсточникДанныхТекущегоВарианта(Вариант)
	Если Не ЗначениеЗаполнено(Вариант) Тогда 
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	Отчеты.УниверсальныйОтчет.УстановитьИсточникДанныхВарианта(Вариант);
КонецПроцедуры
